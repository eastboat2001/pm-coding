export function planAgentV2RepairActions(input) {
    ...
    return uniqueFailures.map((failure) => repairActionForFailure(input.taskId, failure, input.previousFingerprintAttempts?.[failure.fingerprint] ?? 0, input.deliveryMode));
}
function repairActionForFailure(taskId, failure, previousFingerprintAttempts, deliveryMode) {
...
function requiresFullRegeneration(code, deliveryMode) {
    return (code === "static.workspace_empty" ||
...
function reasonForFailure(failure) {
    ...
    if (failure.code === "static.canvas_css_bitmap_mismatch" ||
    ...
    if (failure.code === "static.blueprint_chart_drilldown_incomplete") {
        const missingTargets = Array.isArray(failure.data?.missingTargets)
            ? failure.data.missingTargets.filter((value) => typeof value === "string").slice(0, 6)
            : [];
