...
const MAX_SOURCE_CODE_UNITS = MAX_JSON_CODE_UNITS_PER_SCALAR *
...
export function parseAgentV2ImplementationResult(text, expectedTaskId) {
...
export function parseAgentV2RepairResult(text, expectedTaskId) {
    ...
    if (addressedDiagnosticIds.length === 0 ||
    ...
    const patchPathKeys = new Set();
    for (const patch of patches) {
        const key = patch.path.toLocaleLowerCase("en-US");
        if (changedPathKeys.has(key))
    ...
    for (const key of patchPathKeys)
...
function parseDeletedPaths(value) {
...
function parseRepairPatches(value) {
    ...
    return rawPatches.map((candidate) => {
        ...
        const oldText = inspectBoundedScalarText(patch.oldText, AGENT_V2_REPAIR_WORKSPACE_LIMITS.maxContextBytesPerFile, "limit_exceeded").text;
        if (oldText.length === 0)
            throw new AgentV2ModelContractError("invalid_schema");
        const newText = inspectBoundedScalarText(patch.newText, AGENT_V2_MODEL_RESULT_LIMITS.maxFileContentChars, "limit_exceeded").text;
...
function responseJsonCandidates(source) {
    const candidates = [source];
    const fenced = /^```json\s*\r?\n([\s\S]*?)\r?\n```$/iu.exec(source);
    if (fenced?.[1])
        candidates.push(fenced[1].trim());
    const object = singleBalancedJsonObjectCandidate(source);
    if (object && !candidates.includes(object))
        candidates.push(object);
    return candidates;
}
function singleBalancedJsonObjectCandidate(source) {
    let start = -1;
...
function parseCommonResult(value, expectedTaskId, allowEmptyFiles = false) {
    ...
    const summary = inspectBoundedScalarText(value.summary, AGENT_V2_MODEL_RESULT_LIMITS.maxSummaryChars, "limit_exceeded");
    ...
    const collisionKeys = new Set();
    const files = rawFiles.map((candidate) => {
        ...
        const content = inspectBoundedScalarText(file.content, Math.min(AGENT_V2_MODEL_RESULT_LIMITS.maxFileContentChars, remainingAggregate), "limit_exceeded");
...
function normalizeGeneratedPath(value) {
    ...
    const canonical = inspectBoundedScalarText(raw.text.normalize("NFC"), AGENT_V2_MODEL_RESULT_LIMITS.maxPathChars, "unsafe_path").text;
...
function requireStableIdentifier(value) {
...
function inspectBoundedScalarText(value, maxChars, limitCode) {
    if (typeof value !== "string")
        throw new AgentV2ModelContractError("invalid_schema");
    let length = 0;
    ...
    for (const scalar of value) {
        ...
        length += 1;
        if (length > maxChars)
            throw new AgentV2ModelContractError(limitCode);
        if (!isEcmaWhitespace(scalar.codePointAt(0) ?? 0))
...
//# sourceMappingURL=agent-v2-model-execution.js.map