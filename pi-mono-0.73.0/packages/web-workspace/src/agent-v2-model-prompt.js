...
const REPAIR_PATCH_ONLY_SCHEMA = '{"version":1,"taskId":"<expected task id>","summary":"<summary>","files":[],"patches":[{"path":"<relative path>","expectedChecksum":"sha256:<64 hex>","oldText":"<exact unique text>","newText":"<replacement>"}],"addressedDiagnosticIds":["<diagnostic id>"]}';
...
function renderPrompt(input, mode) {
    ...
    const schema = mode === "repair" ? repairResponseSchema(input) : IMPLEMENTATION_SCHEMA;
...
function repairResponseSchema(input) {
...
function validateRepairPromptIdentity(input, indexedArtifactIds) {
    ...
    const contractRecoveryAttempt = task.input.contractRecoveryAttempt === undefined
        ? 0
        : promptPositiveInteger(task.input.contractRecoveryAttempt);
    const baseRepairTaskId = `repair:${baseValidationTaskId}:${String(validationAttempt)}`;
    const expectedTaskId = contractRecoveryAttempt === 0
        ? baseRepairTaskId
...
function diagnosticFailureCodes(diagnostic, blockingOnly = false) {
    ...
    if (!blockingOnly || !Array.isArray(diagnostic.data.failureDetails))
        return uniqueCodes;
    const blockingCodes = new Set(diagnostic.data.failureDetails
        .filter((item) => isPlainRecord(item) && item.blocking !== false)
        .map((item) => item.code)
        .filter((code) => typeof code === "string"));
    return uniqueCodes.filter((code) => blockingCodes.has(code));
}
function diagnosticFailureDetails(diagnostic, blockingOnly = false) {
...
function diagnosticFailureEvidence(value) {
    if (value === undefined)
        return [];
