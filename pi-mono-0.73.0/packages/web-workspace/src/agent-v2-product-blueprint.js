...
const CATEGORY_PATTERNS = {
    ...
    interaction: /(?:filter|select|click|drill|export|download|sort|pagination|search|tab|筛选|选择|点击|下钻|导出|下载|排序|分页|搜索|切换)/iu,
    ...
    visual: /(?:visual|layout|color|typography|spacing|responsive|mobile|desktop|chart|kpi|design|视觉|布局|颜色|字体|间距|响应式|移动端|桌面端|图表|设计)/iu,
...
export function buildAgentV2ProductBlueprint(input) {
    ...
    for (const candidate of candidates) {
        ...
        if (seenIds.has(id))
            continue;
        if (items.length >= MAX_ITEMS_TOTAL - FALLBACK_ITEM_RESERVE ||
    ...
    if (!items.some((item) => item.categories.includes("acceptance"))) {
        items.push(fallbackItem("acceptance", "Delivered behavior must satisfy the source-backed requirements and platform contract."));
    }
    const categoryItemIds = Object.fromEntries(CATEGORIES.map((category) => [
        category,
...
function fallbackItem(category, text) {
...
function stableItemId(path, text) {
...
function blueprintSummary(objective, lines) {
...
//# sourceMappingURL=agent-v2-product-blueprint.js.map