...
8. User attachments are saved into the current session project workspace. For document attachments, use the listed project workspace paths such as attachments/*.md or docs/*.md and call project_file get to read them; do not rely on duplicate inline document text. Image attachments may also include visual message content so you can inspect them visually, while their original files are retained under attachments/original/ when available.
