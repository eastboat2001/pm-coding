...
import type { AgentV2SkillInstructionContext } from "./agent-v2-model-execution.js";
import type { WorkspaceSkillService } from "./workspace-skill-service.js";
...
export type AgentV2SkillContextErrorCode =
...
export class AgentV2SkillContextError extends Error {
	constructor(readonly code: AgentV2SkillContextErrorCode) {
		super(
...
export function loadAgentV2SkillContext(input: {
	...
	let resourceChars = 0;
	const skills: AgentV2SkillInstructionContext["skills"] = [];
	const resources: AgentV2SkillInstructionContext["resources"] = [];
	for (const name of names) {
		let loaded: ReturnType<WorkspaceSkillService["load"]>;
		try {
			loaded = input.skills.load({ name });
		} catch {
			throw new AgentV2SkillContextError("skill_load_failed");
		}
		if (loaded.name !== name || !loaded.content.trim()) throw new AgentV2SkillContextError("skill_load_failed");
		instructionChars += loaded.content.length;
		if (instructionChars > MAX_INSTRUCTION_CHARS) throw new AgentV2SkillContextError("skill_limit_exceeded");
		skills.push({ name: loaded.name, location: loaded.location, content: loaded.content });

		for (const resource of loaded.resources) {
			if (!loaded.content.includes(resource.path)) continue;
			if (resources.length >= MAX_RESOURCES) throw new AgentV2SkillContextError("skill_limit_exceeded");
			let loadedResource: ReturnType<WorkspaceSkillService["readResource"]>;
			try {
			...
			if (resourceChars > MAX_RESOURCE_TOTAL_CHARS) throw new AgentV2SkillContextError("skill_limit_exceeded");
			resources.push({
				skillName: name,
