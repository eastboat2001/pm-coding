...
const DEFAULT_BUILD_PROXY_IMAGE = "ubuntu/squid@sha256:6a097f68bae708cedbabd6188d68c7e2e7a38cedd05a176e1cc0ba29e3bbe029";
...
export class RetiredSkillConfigError extends Error {
    variable = "PI_DEFAULT_SKILLS_DIR";
    constructor() {
        super("Retired skill configuration is not supported: PI_DEFAULT_SKILLS_DIR. Move each skill directory under PI_SKILLS_DIR and control implicit invocation with agents/openai.yaml.");
...
function digestImageValue(value, variableName) {
    if (!DIGEST_IMAGE.test(value))
        throw invalidContainerBuildConfig(variableName);
    return value;
}
function positiveConfigNumber(value, defaultValue, variableName, requireInteger) {
    if (value === undefined)
        return defaultValue;
...
function registryOriginValues(value) {
