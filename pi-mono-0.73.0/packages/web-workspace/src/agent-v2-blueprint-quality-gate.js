...
const DEFERRED_REQUIREMENT_PATTERN = /(?:optional|fallback|future|nice to have|if approved|confirmation needed|open question|tbd|可选|备选|后续|待定|确认后|开放问题)/iu;
...
const DISTINCT_NAMED_CHART_PATTERN = /^(?:\*\*|__)?((?:donut|pie|pareto|scatter|heatmap|gauge|sankey|treemap|funnel|choropleth|map|network|diagram|timeline|calendar|matrix)(?:\s+(?:chart|graph|plot|view|visuali[sz]ation)))\s*[:：](?:\*\*|__)?/iu;
const VISUALIZATION_SEMANTIC_PATTERN = /(?:chart|graph|plot|trend|donut|pie|pareto|scatter|heatmap|gauge|sankey|treemap|funnel|choropleth|map|network|diagram|timeline|calendar|matrix|visuali[sz]ation|\bviz\b|图表|趋势|环图|饼图|帕累托|热力图|仪表|桑基|树图|漏斗|地图|网络|关系图|时间线|日历|矩阵|可视化)/iu;
const GENERIC_SURFACE_TOKENS = new Set(["chart", "charts", "dashboard", "panel", "view", "图表", "看板", "面板"]);
const EXPLICIT_DETAIL_TABLE_PATTERN = /(?:detail(?:ed)?\s+(?:data\s+)?(?:table|grid)|data\s+grid|明细(?:数据)?(?:表格|列表)|数据表格)/iu;
export function inspectAgentV2BlueprintQuality(input) {
    if (!input.blueprint || input.blueprint.sourceDocuments.length === 0)
        return [];
    ...
    const missingCharts = chartRequirements.filter((requirement) => !sourceContainsNamedSurface(combinedSource, requirement.name) ||
    ...
    if (missingCharts.length > 0) {
        issues.push({
            ...
            message: `Source-backed chart inventory is missing required named chart surfaces: ${missingCharts
                ...
                .join(", ")}.`,
            path: targetPath,
            data: evidenceData(missingCharts, "requiredCharts", missingCharts.map((requirement) => requirement.name)),
        });
    }
    ...
    if (tableRequirements.length > 0 && !sourceContainsDataTable(combinedSource)) {
        issues.push({
            ...
            path: targetPath,
            data: evidenceData(tableRequirements, "requiredTables", tableRequirements.map((requirement) => requirement.name)),
        });
    }
    ...
    if (universalDefaults.length > 0 && !sourceContainsUniversalDefault(combinedSource)) {
        issues.push({
            ...
            path: targetPath,
            data: evidenceData(universalDefaults, "requiredFilters", universalDefaults.map((requirement) => requirement.columns[0] ?? "filter")),
        });
    }
    ...
    if (missingFilters.length > 0) {
        issues.push({
            ...
            message: `Source-backed filter inventory is missing required controls: ${missingFilters
                ...
                .join(", ")}.`,
            path: targetPath,
            data: evidenceData(missingFilters, "requiredFilters", missingFilters.map((requirement) => requirement.columns[0] ?? "filter")),
        });
    }
    ...
    if (singleOptionFilters.length > 0) {
        issues.push({
            ...
            message: `Source-backed interactive dropdowns contain no enabled alternative to their default: ${singleOptionFilters
                ...
                .join(", ")}.`,
            path: targetPath,
            data: evidenceData(singleOptionFilters, "requiredFilters", singleOptionFilters.map((requirement) => requirement.columns[0] ?? "filter")),
        });
    }
    ...
    if (incompleteFilterScope) {
        issues.push({
            ...
            message: "Source-backed filters apply to all charts, but the shared render function derives filtered rows for only part of the chart inventory.",
            path: incompleteFilterScope.path,
            data: {
                ...evidenceData(incompleteFilterScope.requirements, "requiredFilters", incompleteFilterScope.requirements.map((requirement) => requirement.columns[0] ?? "filter")),
    ...
    const interactiveCharts = chartRequirements.filter((requirement) => CHART_INTERACTION_PATTERN.test(requirement.interaction));
    const chartsMissingInteraction = interactiveCharts.filter((requirement) => !sourceContainsChartInteractionForRequirement(combinedSource, requirement));
    if (chartsMissingInteraction.length > 0) {
        ...
        issues.push({
            ...
            message: "Source-backed chart inventory requires click/tap drill-down behavior, but project source contains no chart-segment interaction handler.",
            path: targetPath,
            data: {
                ...evidenceData(chartsMissingInteraction, "interactiveCharts", chartsMissingInteraction.map((requirement) => requirement.name)),
                highConfidence,
            },
        });
    }
    ...
    if (incompleteDrilldowns.length > 0) {
        const proofSource = sources.find((source) => incompleteDrilldowns.some(({ requirement }) => sourceIncompleteDrilldownTargets(source.content, requirement)));
        ...
        issues.push({
            ...
            data: {
                ...evidenceData(requirements, "interactiveCharts", requirements.map((requirement) => requirement.name)),
                missingTargets,
...
function structuredRows(blueprint) {
    return blueprint.items.flatMap((item) => {
        if (DEFERRED_REQUIREMENT_PATTERN.test(item.text))
            return [];
...
function chartRequirementFor(row) {
    ...
    if (!CHART_ID_PATTERN.test(id) || !name || !type)
        return [];
    ...
    if (/(?:table|grid|detail|表格|明细)/iu.test(type) || /[/]/u.test(type))
        return [];
    return [{ ...row, id, name, type, interaction }];
}
function tableRequirementFor(row) {
    const [id = "", name = "", , interaction = ""] = row.columns;
    if (!name || !CHART_ID_PATTERN.test(id))
        return [];
    ...
    if (!EXPLICIT_DETAIL_TABLE_PATTERN.test(interaction))
        return [];
    if (!/(?:click|tap|filter|update|refresh|drill|show|display|点击|点选|筛选|更新|刷新|下钻|展示)/iu.test(interaction)) {
        return [];
    }
    return [{ ...row, name: `${name} detail table` }];
}
function distinctNamedChartRequirementFor(item) {
    if (DEFERRED_REQUIREMENT_PATTERN.test(item.text))
        return [];
...
function uniqueChartRequirements(requirements) {
    return [
        ...new Map(requirements.map((requirement) => [normalizeSearchText(requirement.name), requirement])).values(),
    ];
}
function uniqueTableRequirements(requirements) {
    return [
        ...new Map(requirements.map((requirement) => [normalizeSearchText(requirement.name), requirement])).values(),
    ];
}
function isUniversalDefaultRow(row) {
...
function isFilterInventoryRow(row) {
...
function sourceContainsNamedSurface(source, name) {
    ...
    if (tokens.length === 0)
        return true;
    if (tokens.length === 1)
        return normalizedSource.split(" ").includes(tokens[0] ?? "");
...
function sourceContainsUnimplementedChartPlaceholder(source, requirement) {
    const semanticTokens = uniqueTokens(normalizeSearchText(`${requirement.name} ${requirement.type}`)).filter((token) => !GENERIC_SURFACE_TOKENS.has(token) &&
        /(?:donut|pie|pareto|scatter|heatmap|gauge|sankey|treemap|funnel|choropleth|map|network|diagram|timeline|calendar|matrix|visuali[sz]ation|viz|trend|defect|department|yield|loss|ratio|环|饼|帕累托|热力|仪表|桑基|树图|漏斗|地图|网络|关系|时间线|日历|矩阵|可视化|趋势|缺陷|部门|良率)/iu.test(token));
    ...
    const emptyContainers = [...source.matchAll(/<(?:div|section|article)\b([^>]*)>\s*<\/(?:div|section|article)>/giu)]
        ...
        identity: normalizeSearchText(`${(match[1] ?? "").match(/\bid\s*=\s*(["'])([^"']+)\1/iu)?.[2] ?? ""} ${(match[1] ?? "").match(/\bclass\s*=\s*(["'])([^"']+)\1/iu)?.[2] ?? ""}`),
    }))
        .filter((container) => container.id && VISUALIZATION_SEMANTIC_PATTERN.test(container.identity));
    const target = emptyContainers.find((container) => semanticTokens.some((token) => container.identity.split(" ").includes(token) || container.identity.includes(token)));
    if (!target || sourceReferencesElementId(source, target.id))
        return false;
    ...
    return emptyContainers.some((container) => container.id !== target.id && sourceReferencesElementId(source, container.id));
}
function sourceReferencesElementId(source, id) {
    const escapedId = escapeRegExp(id);
    return new RegExp(String.raw `(?:getElementById\s*\(\s*(["'])${escapedId}\1\s*\)|querySelector(?:All)?\s*\(\s*(["'])#${escapedId}\2\s*\))`, "iu").test(source);
}
function sourceContainsUniversalDefault(source) {
    return /(?:^|[^\p{L}\p{N}_])(?:all|全部|全选)(?:[^\p{L}\p{N}_]|$)/iu.test(source);
}
function sourceContainsExactLabel(source, label) {
    const normalizedLabel = normalizeSearchText(label);
    return !normalizedLabel || normalizeSearchText(source).includes(normalizedLabel);
}
function sourceContainsOnlyOneStaticOption(source, label) {
    ...
    if (/(?:createElement\s*\(\s*["']option["']\s*\)|new\s+Option\s*\(|\.add\s*\(\s*(?:new\s+Option|[A-Za-z_$][\w$]*Option)\b)/iu.test(source)) {
        return false;
    }
    const normalizedLabel = normalizeSearchText(label);
    for (const match of source.matchAll(/<select\b([^>]*)>([\s\S]*?)<\/select>/giu)) {
        ...
        const associatedLabel = (id
            ? source.match(new RegExp(`<label\\b[^>]*\\bfor\\s*=\\s*["']${escapeRegExp(id)}["'][^>]*>([\\s\\S]*?)<\\/label>`, "iu"))?.[1]
            : "") ?? "";
        ...
        if (![associatedLabel, immediatelyPrecedingLabel, wrappingLabel, controlIdentity].some((candidate) => normalizeSearchText(candidate).includes(normalizedLabel))) {
            continue;
        }
        const enabledOptions = [...optionsMarkup.matchAll(/<option\b([^>]*)>/giu)].filter((option) => !/\bdisabled\b/iu.test(option[1] ?? ""));
...
function sourceContainsDataTable(source) {
    return (/<table\b/iu.test(source) ||
...
function sourceContainsChartInteraction(source) {
    ...
    const semanticInlineMarkInteraction = /<(?:div|span|li|tr|td|button|a)\b[^>]{0,640}\b(?:onclick|onpointerup)\s*=\s*["'][^"']{0,240}\b(?:select|drill|filter|show|update)(?:Week|Date|Period|Defect|Code|Point|Segment|Slice|Bar|Trend|Chart|Pareto|Department|Dept|Region|Map|Node|Link|Gauge|Network|Diagram|Timeline|Calendar|Matrix)\s*\(/iu;
    const semanticElementAliasInteraction = new RegExp(String.raw `\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:document\s*\.\s*)?(?:getElementById|querySelector)\s*\(\s*["'][^"']*${subject}[^"']*["']\s*\)[\s\S]{0,4096}?\b\1\s*\.\s*(?:addEventListener\s*\(\s*["'](?:click|pointerup)["']|(?:onclick|onpointerup)\s*=)`, "iu");
    const dynamicSvgMarkInteraction = /\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:document\s*\.\s*)?createElementNS\s*\([\s\S]{0,240}?["'](?:rect|path|circle|line|polyline|polygon)["']\s*\)[\s\S]{0,4096}?\b\1\s*\.\s*(?:addEventListener\s*\(\s*["'](?:click|pointerup)["']|(?:onclick|onpointerup)\s*=)/iu;
    return (new RegExp(`${receiver}\\s*\\.addEventListener\\s*\\(\\s*["'](?:click|pointerup)["']`, "iu").test(source) ||
        new RegExp(`${receiver}\\s*\\.(?:onclick|onpointerup)\\s*=`, "iu").test(source) ||
        new RegExp(`${receiver}\\s*\\.on\\s*\\(\\s*["'](?:click|pointerup)["']`, "iu").test(source) ||
        new RegExp(`(?:select|selectAll)\\s*\\(\\s*["'][^"']*${subject}[^"']*["']\\s*\\)[\\s\\S]{0,80}\\.on\\s*\\(\\s*["']click["']`, "iu").test(source) ||
        new RegExp(`<(?:${subject})\\b[^>]*\\bonClick\\s*=`, "iu").test(source) ||
        /(?:chart|options|plugins)[\s\S]{0,120}\bonClick\s*:/iu.test(source) ||
        new RegExp(`addEventListener\\s*\\(\\s*["'](?:click|pointerup)["'][\\s\\S]{0,320}(?:closest|matches)\\s*\\([^)]*${subject}`, "iu").test(source) ||
...
function sourceContainsChartInteractionForRequirement(source, requirement) {
    const semanticText = normalizeSearchText(`${requirement.name} ${requirement.type}`);
    const semanticTokens = uniqueTokens(semanticText).filter((token) => !GENERIC_SURFACE_TOKENS.has(token) &&
        !["bar", "line", "horizontal", "vertical", "ratio", "loss", "overall"].includes(token));
    if (/\bdefect\b.*\bloss\b|\bloss\b.*\bdefect\b/iu.test(semanticText))
        semanticTokens.push("pareto");
    ...
    if (segments.length === 0)
        return sourceContainsChartInteraction(source);
    return (segments.some((segment) => sourceContainsChartInteraction(segment.source) ||
        segmentContainsSelectedStateInteraction(segment.source) ||
        segmentUsesProvenInteractionForwarder(source, segment.source) ||
        segmentUsesProvenSvgMarkFactory(source, segment.source)) || sourceContainsInlineInteractionForRequirement(source, semanticTokens));
}
function sourceContainsAnyPointerInteraction(source) {
    return (/\.\s*(?:onclick|onpointerup)\s*=|\.\s*addEventListener\s*\(\s*["'](?:click|pointerup)["']/iu.test(source) ||
        /\.\s*on\s*\(\s*["'](?:click|pointerup)["']|\bonClick\s*[:=]/iu.test(source) ||
        /\b(?:onclick|onpointerup)\s*=\s*["']/iu.test(source));
}
function segmentContainsSelectedStateInteraction(segment) {
    ...
    const interaction = /\.\s*(?:onclick|onpointerup)\s*=|\.\s*addEventListener\s*\(\s*["'](?:click|pointerup)["']/iu.test(segment);
...
function segmentUsesProvenSvgMarkFactory(source, segment) {
    const declarations = [
        ...source.matchAll(/\bfunction\s+([A-Za-z_$][\w$]*)\s*\(\s*([A-Za-z_$][\w$]*)[^)]*\)\s*\{/gu),
        ...source.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:\(\s*)?([A-Za-z_$][\w$]*)[^=)]*(?:\))?\s*=>\s*\{/gu),
    ];
    for (const declaration of declarations) {
        const helperName = declaration[1];
        const tagParameter = declaration[2];
        if (!helperName || !tagParameter)
            continue;
        const declarationWindow = source.slice(declaration.index ?? 0, (declaration.index ?? 0) + 2_048);
        if (!new RegExp(String.raw `\bcreateElementNS\s*\([^)]{0,512},\s*${escapeRegExp(tagParameter)}\s*\)`, "u").test(declarationWindow)) {
            continue;
        }
        const callPattern = new RegExp(String.raw `\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*${escapeRegExp(helperName)}\s*\(\s*["'](?:rect|path|circle|line|polyline|polygon)["']`, "gu");
        for (const call of segment.matchAll(callPattern)) {
            ...
            if (new RegExp(String.raw `\b${escapeRegExp(markAlias)}\s*\.\s*(?:addEventListener\s*\(\s*["'](?:click|pointerup)["']|(?:onclick|onpointerup)\s*=)`, "u").test(interactionWindow)) {
                return true;
            }
        }
    }
...
function segmentUsesProvenInteractionForwarder(source, segment) {
    for (const helper of source.matchAll(/\bfunction\s+([A-Za-z_$][\w$]*)\s*\(\s*([A-Za-z_$][\w$]*)\s*,\s*([A-Za-z_$][\w$]*)\s*,\s*([A-Za-z_$][\w$]*)\s*\)\s*\{([\s\S]{0,512}?)\}/gu)) {
        const [name, elementParameter, eventParameter, listenerParameter, body] = helper.slice(1);
        if (!name || !elementParameter || !eventParameter || !listenerParameter || !body)
            continue;
        if (!new RegExp(String.raw `\b${escapeRegExp(elementParameter)}\s*\.\s*addEventListener\s*\(\s*${escapeRegExp(eventParameter)}\s*,\s*${escapeRegExp(listenerParameter)}\b`, "u").test(body)) {
            continue;
        }
        if (new RegExp(String.raw `\b${escapeRegExp(name)}\s*\([^;\r\n]{0,320}?["'](?:click|pointerup)["']`, "iu").test(segment)) {
            return true;
        }
    }
    return false;
}
function sourceContainsInlineInteractionForRequirement(source, semanticTokens) {
    for (const element of source.matchAll(/<(?:svg|canvas|div)\b([^>]*)>([\s\S]{0,4096}?)<\/(?:svg|canvas|div)>/giu)) {
        ...
        if (!semanticTokens.some((token) => identity.includes(token)))
            continue;
...
function declaredFunctionSegments(source) {
    const declarations = [
...
function sourceProvesAllChartsFilterScopeIncomplete(sources, filterRequirements) {
    const requirements = filterRequirements.filter((requirement) => /^(?:all\s+charts?|all\s+visuals?|所有图表|全部图表)$/iu.test(requirement.columns[3] ?? ""));
    ...
    for (const projectSource of sources) {
        ...
        for (const segment of segments) {
            ...
            const filteredAlias = segment.source.match(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:getFilteredData|deriveFilteredData|filterData|applyFiltersToData)\s*\(/u)?.[1];
            if (!filteredAlias)
                continue;
            const chartCalls = [...segment.source.matchAll(/\b((?:render|draw|update)[A-Za-z_$][\w$]*(?:Chart|Graph|Plot|Trend|Pareto|Donut|Heatmap|Treemap|Funnel|Sankey|Map|Gauge|Network|Diagram|Timeline|Calendar|Matrix|Visualization|View))\s*\(([^)]*)\)/gu)].map((call) => ({ name: call[1] ?? "", arguments: call[2] ?? "" }));
            if (chartCalls.length < 2)
                continue;
            const escapedAlias = escapeRegExp(filteredAlias);
            const filteredCalls = chartCalls.filter((call) => {
                if (new RegExp(String.raw `\b${escapedAlias}\b`, "u").test(call.arguments))
                    return true;
                if (/(?:getFilteredData|state\s*\.\s*filters|\bfilters\b)/iu.test(call.arguments))
                    return true;
            ...
            if (filteredCalls.length === 0)
                continue;
            const unfilteredCharts = chartCalls
                .filter((call) => !filteredCalls.includes(call))
                .map((call) => call.name);
            if (unfilteredCharts.length === 0)
                continue;
...
function sourceContainsForwardedChartInteraction(source, subject) {
    const forwardingHelpers = new Set();
    const helperPattern = /\bfunction\s+([A-Za-z_$][\w$]*)\s*\(\s*([A-Za-z_$][\w$]*)\s*,\s*([A-Za-z_$][\w$]*)\s*,\s*([A-Za-z_$][\w$]*)\s*\)\s*\{([\s\S]{0,512}?)\}/gu;
    for (const helper of source.matchAll(helperPattern)) {
        ...
        if (new RegExp(String.raw `\b${escapedElement}\s*\.\s*addEventListener\s*\(\s*${escapedEvent}\s*,\s*${escapedListener}\b`, "u").test(body)) {
    ...
    for (const helperName of forwardingHelpers) {
        ...
        if (new RegExp(String.raw `\b${escapedHelper}\s*\(\s*${semanticReceiver}\s*,\s*["'](?:click|pointerup)["']\s*,`, "iu").test(source)) {
            return true;
        }
    }
    return false;
}
function sourceIncompleteDrilldownTargets(source, requirement) {
    const semanticTokens = uniqueTokens(normalizeSearchText(requirement.name)).filter((token) => !GENERIC_SURFACE_TOKENS.has(token) && !["loss", "ratio", "overall"].includes(token));
    if (semanticTokens.length === 0)
        return undefined;
    ...
    for (const assignment of source.matchAll(assignmentPattern)) {
        ...
        if (!qualifiedName || !selectionTokens.some((token) => semanticallyOverlaps(selectedName, token)))
            continue;
        ...
        for (const occurrence of occurrences) {
            ...
            if (ignoredOccurrences.has(occurrenceIndex))
                continue;
            ...
            if (!scope || /(?:reset|initiali[sz]e|setup|bootstrap)/iu.test(scope))
                continue;
            const normalizedScope = normalizeSearchText(scope);
            if (semanticTokens.some((token) => normalizedScope.includes(token)) ||
...
function sourceIgnoredDownstreamSelectionOccurrences(source, semanticTokens) {
    const ignoredOccurrences = new Set();
    for (const ignoredFilter of source.matchAll(/(?:^|[;{}\r\n])\s*(?:if\s*\([^;\r\n]{0,256}\)\s*)?[A-Za-z_$][\w$]*\s*\.\s*filter\s*\([^;\r\n]{0,512}\bstate\s*\.\s*selected([A-Za-z_$][\w$]*)\b[^;\r\n]{0,256}\)\s*;/gmu)) {
        ...
        if (!semanticTokens.some((token) => semanticallyOverlaps(selectedName, token)))
            continue;
...
function occurrenceIsDownstreamCallArgument(source, index, semanticTokens, targetTokens) {
    ...
    if (semanticTokens.some((token) => normalizedCall.includes(token)))
        return false;
    if (targetTokens.some((token) => normalizedCall.includes(token)))
        return true;
    return /(?:synchroni[sz]e|drilldown|applySelectionTo|refreshDependent)/iu.test(callName.replace(/\s+/gu, ""));
}
function downstreamTargetGroups(requirement) {
    const targetClause = requirement.interaction.match(/(?:update|refresh|filter|drill(?:-?down)?(?:\s+into)?|show|display|load|synchroni[sz]e|同步|联动|更新|刷新|筛选|下钻|展示|显示)\s*(?:the\s+)?([^.;；。]{2,160})/iu)?.[1];
    if (!targetClause)
        return [];
    const rawTargets = targetClause
        ...
        .filter(Boolean);
    const groups = rawTargets.flatMap((label) => {
        const tokens = uniqueTokens(label).filter((token) => !["the", "all", "related", "corresponding", "相关", "对应"].includes(token));
        if (tokens.length === 0)
            return [];
...
function occurrenceParticipatesInDataSelection(source, index) {
    ...
    if (/(?:style|class(?:Name)?|stroke|border|highlight|selected)\s*[=:][\s\S]{0,180}$/iu.test(window.slice(0, relativeIndex))) {
        return false;
    }
    ...
    if (localAlias) {
        ...
        if (new RegExp(String.raw `[;\r\n][\s\S]{0,240}\.(?:filter|find|findIndex|some|every|reduce|groupBy)\s*\([\s\S]{0,240}\b${escapedAlias}\b`, "u").test(aliasTail)) {
            return true;
        }
    }
    return (/\.(?:filter|find|findIndex|some|every|reduce|groupBy)\s*\([\s\S]{0,360}$/u.test(window.slice(0, relativeIndex)) ||
        /\[[\s\S]{0,80}(?:state|selection|filters)\s*\./u.test(window) ||
        /(?:rows|records|data|items|series|details|defects|departments|regions|nodes|links|categories)\s*=\s*[^;\r\n]{0,240}(?:state|selection|filters)\s*\./iu.test(window));
}
function selectionStateTokens(requirement, semanticTokens) {
    ...
    if (/(?:defect|loss|pareto|缺陷|损失|帕累托)/iu.test(identity)) {
        for (const token of ["defect", "loss", "code", "bar", "index", "缺陷", "损失", "代码"])
            tokens.add(token);
    }
    return [...tokens];
}
function sourceProvesInstanceLocalHighlight(source, semanticTokens) {
    ...
    for (const assignment of source.matchAll(assignmentPattern)) {
        ...
        if (!semanticTokens.some((token) => selectedName.includes(token) || chartName.includes(token)))
            continue;
        const assignmentIndex = assignment.index ?? 0;
        const handlerEnd = source.indexOf("});", assignmentIndex);
        if (handlerEnd < 0 || handlerEnd - assignmentIndex > 1_200)
            continue;
        ...
        if (!ownRedrawPattern.test(handlerTail))
            continue;
...
function enclosingFunctionName(source, index) {
    ...
    const functions = [
        ...
        ...before.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:async\s*)?(?:\([^)]*\)|[A-Za-z_$][\w$]*)\s*=>\s*\{/gu),
    ].sort((left, right) => (left.index ?? 0) - (right.index ?? 0));
    return functions.at(-1)?.[1];
}
function evidenceData(requirements, valueKey, values) {
...
function preferredTargetPath(sources) {
    return (sources.find((source) => /(?:^|\/)index\.html$/iu.test(source.path))?.path ?? sources[0]?.path ?? "index.html");
}
...
function hasNearbyTokenCoverage(source, tokens, required, window) {
...
//# sourceMappingURL=agent-v2-blueprint-quality-gate.js.map