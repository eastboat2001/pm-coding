...
const DEDICATED_CHART_CONTAINER_SELECTOR_PATTERN = /(?:chart|graph|plot|visuali[sz]ation)(?:[-_ ]?(?:container|wrapper|viewport|frame|panel|region|area))|(?:container|wrapper|viewport|frame|panel|region|area)(?:[-_ ]?(?:chart|graph|plot|visuali[sz]ation))/iu;
const BOUNDED_CHART_SIZE_PATTERN = /(?:^|;)\s*(?:height|max-height|aspect-ratio)\s*:\s*(?!(?:auto|inherit|initial|unset|100%)\b)(?:clamp\(|min\(|max\(|\d+(?:\.\d+)?(?:px|rem|em|vh|vw)\b)/iu;
const BOUNDED_CHART_SIZE_VARIABLE_PATTERN = /(?:height|max-height|aspect-ratio)\s*:\s*var\(\s*(--[\w-]+)(?:\s*,[^)]*)?\s*\)/giu;
...
const GENERIC_SELECT_VALUE_HANDLER_PATTERN = /(?:(?:querySelectorAll)\(\s*(['"`])[^'"`]*\bselect\b[^'"`]*\1\s*\)|(?:getElementsByTagName)\(\s*(['"`])select\2\s*\))[\s\S]{0,2048}(?:\.value\b|target\s*\.\s*value\b)/u;
const GENERIC_SELECT_CHANGE_HANDLER_PATTERN = /(?:(?:querySelectorAll)\(\s*(['"`])[^'"`]*\bselect\b[^'"`]*\1\s*\)|(?:getElementsByTagName)\(\s*(['"`])select\2\s*\))[\s\S]{0,2048}\.addEventListener\s*\(\s*(['"`])change\3/u;
...
const DYNAMIC_SELECT_CHANGE_HANDLER_PATTERN = /\bgetElementById\(\s*[A-Za-z_$][\w$]*\s*\)[\s\S]{0,2048}\baddEventListener\(\s*(['"`])change\1/u;
const RANDOM_RENDERED_DATA_PATTERN = /\bdata\s*:\s*[\s\S]{0,512}\bMath\.random\s*\(/u;
const MOCK_DATA_RANDOM_PATTERN = /(?:\bfunction\s+((?=[A-Za-z_$][\w$]*(?:mock|demo|sample|fixture|seed))[A-Za-z_$][\w$]*)\s*\([^)]*\)\s*\{|\b(?:const|let|var)\s+((?=[A-Za-z_$][\w$]*(?:mock|demo|sample|fixture|seed))[A-Za-z_$][\w$]*)\s*=\s*(?:\([^)]*\)|[A-Za-z_$][\w$]*)\s*=>\s*(?:\{|\[))[\s\S]{0,8192}?\bMath\.random\s*\(/giu;
const OBJECT_DATA_METHOD_RANDOM_PATTERN = /\b((?:generate|create|build|make)(?:[A-Za-z_$][\w$]*)?(?:data|dataset|series|metrics?))\s*:\s*function\s*\([^)]*\)\s*\{[\s\S]{0,8192}?\bMath\.random\s*\(/giu;
const OBJECT_DATA_METHOD_SHORTHAND_PATTERN = /\b((?:generate|create|build|make)(?:[A-Za-z_$][\w$]*)?(?:data|dataset|series|metrics?))\s*\([^)]*\)\s*\{/giu;
const CHART_SEMANTIC_PATTERN = /(?:chart|trend|graph|plot|yield|fpy|defect|scrap|kpi|metric|analytics|series|histogram|scatter|visuali[sz]ation|\bviz\b|heatmap|treemap|choropleth|map|gauge|network|diagram|timeline|calendar|matrix)/iu;
const GAME_OR_DRAWING_CANVAS_PATTERN = /(?:game|arcade|hud|board|stage|sprite|paint|drawing|editor|simulator|游戏|街机|画板|棋盘|模拟器)/iu;
const CANVAS_DRAWING_CALL_PATTERN = /\.(?:moveTo|lineTo|fillText|strokeText|arc|bezierCurveTo|quadraticCurveTo)\s*\(/gu;
const RESPONSIVE_CANVAS_MEASUREMENT_PATTERN = /\.(?:clientWidth|clientHeight|offsetWidth|offsetHeight)\b|\.getBoundingClientRect\s*\(/u;
const CANVAS_RESIZE_OBSERVER_PATTERN = /\bResizeObserver\s*\(/u;
const CANVAS_CARD_SURFACE_CLASS_PATTERN = /(?:^|\s)(?:card|card-body)(?:\s|$)/iu;
const CANVAS_SURFACE_CHROME_CLASS_PATTERN = /(?:^|\s)(?:card-title|chart-title|panel-title|toolbar|actions|header)(?:\s|$)/iu;
const CANVAS_VIEWPORT_OVERLAY_CLASS_PATTERN = /(?:^|\s)(?:tooltip|chart-tooltip|overlay|crosshair|annotation|hover-label|empty-state|chart-empty|no-data)(?:\s|$)/iu;
...
export function assessStaticPreviewQuality(input) {
    const errors = [];
    const warnings = [];
    const checkedFiles = [];
    let guard;
    ...
    for (const loadingId of visibleLoadingIds(html)) {
        if (referencedIds.has(loadingId))
            continue;
    ...
    for (const placeholderId of metricPlaceholderIds(html)) {
        if (referencedIds.has(placeholderId))
            continue;
    ...
    const specificallyDiagnosedSelectIds = new Set(selectValueErrors.flatMap((error) => {
        const id = /\bSelect\s+#([^\s,.:]+)/u.exec(error)?.[1];
        return id ? [id] : [];
    }));
    errors.push(...unwiredSelectErrors(html, scriptSource).filter((error) => {
    ...
    const nondeterministicEvidence = nondeterministicRenderedDataEvidence(scriptSources, /(?:dashboard|analytics|kpi|chart|trend|yield|pareto|data table|看板|分析|指标|图表|趋势|良率)/iu.test(`${html}\n${scriptSource}`));
    if (nondeterministicEvidence) {
        errors.push(`static.nondeterministic_data: Unseeded Math.random() feeds rendered chart or mock/demo application data.${nondeterministicEvidence.highConfidence ? " Context: dashboard-first-render." : ""} Evidence: ${nondeterministicEvidence.evidence}`);
    }
...
function filterStateKeyMismatchErrors(html, sources) {
    const errors = [];
    for (const source of sources) {
        for (const derived of source.content.matchAll(/(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*([A-Za-z_$][\w$]*)\.split\(\s*(['"])-\3\s*\)\s*\[\s*0\s*\]/gu)) {
            if (derived.index === undefined)
                continue;
            const keyVariable = derived[1];
            const idVariable = derived[2];
            if (!keyVariable || !idVariable)
                continue;
            const escapedKey = keyVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            const assignment = new RegExp(`\\b([A-Za-z_$][\\w$]*)\\s*\\[\\s*${escapedKey}\\s*\\]\\s*=`, "u").exec(source.content.slice(derived.index, derived.index + 768));
            const stateVariable = assignment?.[1];
            if (!stateVariable)
                continue;
            const escapedState = stateVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            const stateBody = new RegExp(`(?:const|let|var)\\s+${escapedState}\\s*=\\s*\\{([\\s\\S]{0,8192}?)\\}\\s*;`, "u").exec(source.content)?.[1];
            if (!stateBody)
                continue;
            const stateKeys = new Set([...stateBody.matchAll(/(?:^|[,\n])\s*([A-Za-z_$][\w$]*)\s*:/gu)].map((match) => match[1] ?? ""));
            ...
            if (!new RegExp(`\\.forEach\\(\\s*${escapedIdVariable}\\s*=>`, "u").test(surrounding))
                continue;
            const controls = unique([...source.content.matchAll(/(['"])([A-Za-z][\w-]*-[A-Za-z][\w-]*-filter)\1/gu)]
                .map((match) => match[2] ?? "")
                .filter((id) => selectIdExists(html, id)));
            const affected = controls.flatMap((id) => {
                const parts = id.split("-").filter(Boolean);
                if (parts.at(-1)?.toLowerCase() !== "filter" || parts.length < 3)
                    return [];
            ...
            if (affected.length === 0)
                continue;
            errors.push(`static.filter_state_key_mismatch: Shared select handler derives only the prefix before "-" and writes nonexistent state keys, so affected filters cannot update their declared render state. Controls: ${affected.join(", ")}. Evidence: ${evidenceAt(source, derived.index)}`);
        }
    }
    return unique(errors);
}
function selectIdExists(html, id) {
    const escaped = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    return new RegExp(`<select\\b(?=[^>]*\\bid\\s*=\\s*(['"])${escaped}\\1)[^>]*>`, "iu").test(html);
}
function nativeSvgChartIssues(html, scriptSources, stylesheets) {
    const styleSource = combinedStyleSource(html, stylesheets);
    const svgElements = staticMarkupElements(html).filter((element) => element.tagName === "svg" &&
        element.id &&
        CHART_SEMANTIC_PATTERN.test(`${element.id} ${element.classNames.join(" ")} ${element.parent?.id ?? ""} ${element.parent?.classNames.join(" ") ?? ""}`));
    ...
    for (const svg of svgElements) {
        ...
        if (!mismatch)
            continue;
        errors.push(`static.svg_coordinate_space_mismatch: Responsive chart SVG #${svg.id} fills a ${viewportHeight}px viewport but is drawn with a fixed ${mismatch.logicalHeight}px coordinate height without a matching viewBox or measured CSS height; marks can be clipped or stretched. Evidence: ${mismatch.evidence}`);
    }
    return { errors, warnings };
}
function explicitPixelHeight(declarations) {
    const values = [...declarations.matchAll(/(?:^|;)\s*height\s*:\s*(\d+(?:\.\d+)?)px\b/giu)];
    const value = Number(values.at(-1)?.[1]);
    return Number.isFinite(value) && value > 0 ? value : undefined;
}
function svgFillsViewport(svg, styleSource) {
    const declarations = elementDeclarations(svg, styleSource);
    return (/(?:^|;)\s*width\s*:\s*100%(?:\s*!important)?(?:;|$)/iu.test(declarations) &&
        /(?:^|;)\s*height\s*:\s*100%(?:\s*!important)?(?:;|$)/iu.test(declarations));
}
function svgHasViewBox(html, id, sources) {
    const escaped = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const markup = new RegExp(`<svg\\b(?=[^>]*\\bid\\s*=\\s*(['"])${escaped}\\1)[^>]*>`, "iu").exec(html)?.[0] ?? "";
    if (/\bviewBox\s*=\s*(['"])[^'"]+\1/iu.test(markup))
        return true;
    const scriptSource = sources.map((source) => source.content).join("\n");
    return new RegExp(`(?:getElementById\\(\\s*(['"])${escaped}\\1\\s*\\)|[A-Za-z_$][\\w$]*\\(\\s*(['"])${escaped}\\2\\s*\\))[\\s\\S]{0,512}?\\.setAttribute\\(\\s*(['"])viewBox\\3`, "u").test(scriptSource);
}
function responsiveSvgLiteralHeightMismatch(id, viewportHeight, sources) {
    const escaped = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    for (const source of sources) {
        const measurementPattern = new RegExp(`(?:const|let|var)\\s+([A-Za-z_$][\\w$]*)\\s*=\\s*(?:document\\.getElementById\\(\\s*(['"])${escaped}\\2\\s*\\)|[A-Za-z_$][\\w$]*\\(\\s*(['"])${escaped}\\3\\s*\\))\\.getBoundingClientRect\\s*\\(\\s*\\)`, "gu");
        for (const measurement of source.content.matchAll(measurementPattern)) {
            const variable = measurement[1];
            if (!variable || measurement.index === undefined)
                continue;
            ...
            if (!literal || !Number.isFinite(logicalHeight) || Math.abs(logicalHeight - viewportHeight) < 2)
                continue;
...
function windowNamedElementChartErrors(htmlIds, scriptSource) {
    const errors = [];
    for (const match of scriptSource.matchAll(WINDOW_CHART_DESTROY_PATTERN)) {
        const name = match[1] ?? "";
        if (!name || !htmlIds.has(name) || hasSafeChartDestroyGuard(scriptSource, name))
            continue;
        errors.push(`Chart instance window.${name} collides with HTML id #${name}; the browser may expose the element on window before Chart initialization, so destroy() can fail. Use a differently named instance variable or guard destroy with instanceof Chart/typeof.`);
    }
    return unique(errors);
}
function hasSafeChartDestroyGuard(source, name) {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    return (new RegExp(String.raw `window\.${escaped}\s+instanceof\s+Chart`, "u").test(source) ||
        new RegExp(String.raw `typeof\s+window\.${escaped}\.destroy\s*===?\s*['"]function['"]`, "u").test(source));
}
function controlTextContrastIssues(html, stylesheets) {
    const styleSource = [
    ...
    for (const element of staticTextElements(html)) {
        ...
        if (!colors.foreground || !colors.background || colors.foreground.a < 1 || colors.background.a < 1)
            continue;
...
function cssColorRules(source) {
    ...
    for (const match of source.matchAll(CSS_RULE_PATTERN)) {
        ...
        if (!foregroundDeclaration && !backgroundDeclaration)
            continue;
...
function staticTextElements(html) {
    const elements = [];
    for (const match of html.matchAll(OPEN_TAG_PATTERN)) {
        const tagName = (match[1] ?? "").toLowerCase();
        if (!TEXT_CONTROL_TAGS.has(tagName))
            continue;
        const attrs = match[2] ?? "";
...
function resolvedElementColors(element, rules) {
    let foreground;
    let background;
    for (const rule of rules) {
        if (!simpleCssSelectorMatches(element, rule.selector))
            continue;
...
function lastColorDeclaration(declarations, properties) {
    let result;
    for (const property of properties) {
        for (const match of declarations.matchAll(new RegExp(`(?:^|;)\\s*${property}\\s*:\\s*([^;!]+)(?:!important)?`, "giu"))) {
            const value = match[1]?.trim();
            const index = match.index ?? 0;
            if (!value || (result && result.index > index))
                continue;
...
function readLocalStylesheets(guard, html) {
    ...
    for (const match of html.matchAll(LINK_TAG_PATTERN)) {
        ...
        if (!href || reference?.kind !== "local" || !reference.relativePath.toLowerCase().endsWith(".css"))
            continue;
        try {
...
function nativeCanvasChartIssues(html, scriptSources, stylesheets) {
    ...
    const chartCanvases = canvases.filter((canvas) => {
        ...
        return isNativeChartCanvas(canvas, html, drawingCallCount, Boolean(responsiveMeasurement), hasAssociatedScaling, canvases.length);
    });
    ...
    for (const scaling of scalings) {
        const associatedCanvases = scaling.canvasIds.length > 0
            ? chartCanvases.filter((canvas) => scaling.canvasIds.includes(canvas.id))
            : chartCanvases.length === 1
                ? chartCanvases
                : [];
        const canvasesMissingDisplaySize = associatedCanvases.filter((canvas) => !hasExplicitCanvasDisplaySize(canvas, styleSource, scaling));
        if (canvasesMissingDisplaySize.length > 0) {
            errors.push(`static.canvas_css_bitmap_mismatch: Chart bitmap dimensions are multiplied for DPR/a scale factor without explicit CSS display dimensions. Canvases: ${chartCanvasLabels(canvasesMissingDisplaySize)}. Evidence: ${sourceEvidence(scaling.path, scaling.line, scaling.excerpt)}`);
            continue;
        }
        const aspectMismatchedCanvases = associatedCanvases.filter((canvas) => hasResponsiveCanvasBitmapAspectMismatch(canvas, scaling, styleSource));
        if (aspectMismatchedCanvases.length > 0) {
            errors.push(`static.canvas_css_bitmap_mismatch: Chart CSS display dimensions fill a non-square responsive viewport, but both bitmap dimensions use the same fixed logical size; this stretches rendered chart pixels. Canvases: ${chartCanvasLabels(aspectMismatchedCanvases)}. Evidence: ${sourceEvidence(scaling.path, scaling.line, scaling.excerpt)}`);
        }
        if (associatedCanvases.length === 0) {
            const canvasesMissingDisplaySize = chartCanvases.filter((canvas) => !hasExplicitCanvasDisplaySize(canvas, styleSource, scaling));
            if (canvasesMissingDisplaySize.length > 0) {
                warnings.push(`Possible Canvas CSS/bitmap mismatch was not blocked because the scaled canvas variable could not be associated with one of several mixed chart canvases. Canvases needing review: ${chartCanvasLabels(canvasesMissingDisplaySize)}. Evidence: ${sourceEvidence(scaling.path, scaling.line, scaling.excerpt)}`);
            }
        }
    }
    const unscaledResponsiveCanvases = chartCanvases.filter((canvas) => {
        if (scalings.some((scaling) => scalingAppliesToCanvas(scaling, canvas, canvases.length)))
            return false;
        const declarations = elementDeclarations(canvas, styleSource);
        return (cssDimensionIsFullSize(declarations, "width") &&
    ...
    if (unscaledResponsiveCanvases.length > 0) {
        ...
        if (unassociatedScaling) {
            warnings.push(`Responsive chart bitmap sizing could not be associated with specific canvases, so a CSS/bitmap mismatch was not blocked. Canvases needing browser review: ${chartCanvasLabels(unscaledResponsiveCanvases)}. Evidence: ${sourceEvidence(unassociatedScaling.path, unassociatedScaling.line, unassociatedScaling.excerpt)}`);
        }
        else {
            const evidence = firstSourceEvidence(scriptSources, /\.canvas\.(?:width|height)\b/u) ??
                invalidViewportEvidence(unscaledResponsiveCanvases[0]);
            errors.push(`static.canvas_css_bitmap_mismatch: Responsive native chart CSS fills a bounded viewport, but bitmap dimensions are never synchronized to the measured CSS size and devicePixelRatio; the default bitmap will be stretched. Canvases: ${chartCanvasLabels(unscaledResponsiveCanvases)}. Evidence: ${evidence}`);
        }
    }
    const invalidViewportCanvases = chartCanvases.filter((canvas) => !canvas.parent || !isDedicatedBoundedCanvasViewport(canvas.parent, styleSource));
    const uncertainViewportCanvases = invalidViewportCanvases.filter((canvas) => couldBeBoundedByExternalStylesheet(canvas, html));
    const blockingViewportCanvases = invalidViewportCanvases.filter((canvas) => !uncertainViewportCanvases.includes(canvas));
    if (blockingViewportCanvases.length > 0) {
        const evidence = invalidViewportEvidence(blockingViewportCanvases[0]);
        errors.push(`static.canvas_layout_unbounded: Chart canvases must be direct children of dedicated drawing viewports with an explicit height, max-height, or aspect-ratio; min-height alone does not prevent flex/grid stretching. Do not use cards containing titles, padding, or toolbars as the drawing viewport. Canvases: ${chartCanvasLabels(blockingViewportCanvases)}. Evidence: ${evidence}`);
    }
    if (uncertainViewportCanvases.length > 0) {
        warnings.push(`Chart viewport bounds could not be verified because their sizing may come from an external stylesheet. Canvases needing browser review: ${chartCanvasLabels(uncertainViewportCanvases)}.`);
    }
    if (responsiveMeasurement && !CANVAS_RESIZE_OBSERVER_PATTERN.test(maskJavaScriptCommentsAndStrings(scriptSource))) {
        errors.push(`static.canvas_resize_unhandled: Responsive chart canvases measure layout dimensions without observing their drawing viewports with ResizeObserver. A window resize listener alone misses container-only size changes. Canvases: ${canvasIds}. Evidence: ${responsiveMeasurement}`);
    }
    return { errors, warnings };
}
function scalingAppliesToCanvas(scaling, canvas, canvasCount) {
    return scaling.canvasIds.includes(canvas.id) || (scaling.canvasIds.length === 0 && canvasCount === 1);
}
function uniqueStaticCanvasElements(canvases) {
    const seen = new Set();
...
function chartLayoutIssues(html, scripts, stylesheets) {
    const scriptSource = combinedScriptSource(html, scripts);
    if (!CHART_CONSTRUCTOR_PATTERN.test(scriptSource) ||
    ...
    const invalidCanvases = canvases.filter((canvas) => !canvas.parent || !isDedicatedBoundedChartJsViewport(canvas.parent, styleSource));
    if (invalidCanvases.length === 0)
        return { errors: [], warnings: [] };
    ...
    return {
        errors: blockingCanvases.length > 0
            ? [
            ...
            : [],
        warnings: uncertainCanvases.length > 0
            ? [
...
function dashboardIntrinsicWidthIssues(html, scriptSources, stylesheets) {
    ...
    const chartCanvases = nativeChartCanvases(html, scriptSources, elements.filter((element) => element.tagName === "canvas"));
    for (const grid of elements) {
        const declarations = elementDeclarations(grid, styleSource);
        if (!/\bdisplay\s*:\s*grid\b/iu.test(declarations) || !hasExplicitMultiColumnGrid(declarations))
            continue;
        ...
        for (const child of grid.children) {
            ...
            if (couldContainIntrinsicWidthViaExternalStylesheet(html, child, wideTable)) {
                warnings.push(`Dashboard table containment could not be verified because its responsive wrapper may be styled by an external stylesheet. Grid needing browser review: ${gridLabel}.`);
                continue;
            }
            if (!hasExplicitTableWidthPressure(wideTable, styleSource, headerCount)) {
                warnings.push(`A multi-column dashboard grid contains a ${headerCount}-column table without an explicit local overflow wrapper, but source does not prove intrinsic width pressure; browser review is recommended instead of blocking generation. Grid: ${gridLabel}.`);
                continue;
            }
            errors.push(`static.page_horizontal_overflow: A multi-column dashboard grid contains a ${headerCount}-column table whose intrinsic width is not contained. Add min-width:0 to the table's direct grid item and wrap the table in a local overflow-x:auto region. Grid: ${gridLabel}. Evidence: ${gridRuleEvidence(grid, html, stylesheets)}`);
            break;
        }
    }
    return { errors: unique(errors), warnings: unique(warnings) };
}
function nativeChartCanvases(html, scriptSources, markupCanvases = staticCanvasElements(html)) {
    const canvases = uniqueStaticCanvasElements([
        ...markupCanvases,
        ...scriptSources.flatMap((source) => staticCanvasElements(source.content)),
    ]);
    ...
    const scalings = canvasBitmapScalings(scriptSources);
    const responsiveMeasurement = firstSourceEvidence(scriptSources, RESPONSIVE_CANVAS_MEASUREMENT_PATTERN);
    return canvases.filter((canvas) => isNativeChartCanvas(canvas, html, drawingCallCount, Boolean(responsiveMeasurement), scalings.some((scaling) => scalingAppliesToCanvas(scaling, canvas, canvases.length)), canvases.length));
}
function hasUncontainedFractionalGridTracks(declarations) {
    for (const match of declarations.matchAll(/\bgrid-template-columns\s*:\s*([^;]+)/giu)) {
        const value = (match[1] ?? "").trim();
        if (!hasExplicitMultiColumnGrid(`grid-template-columns:${value}`))
            continue;
        const withoutZeroMinimumTracks = value.replace(/\bminmax\(\s*0(?:px|rem|em|%)?\s*,\s*[^)]*\b\d+(?:\.\d+)?fr\b[^)]*\)/giu, "");
        if (/\b\d+(?:\.\d+)?fr\b/iu.test(withoutZeroMinimumTracks))
            return true;
    }
    return false;
}
...
function gridItemCanShrink(element, styleSource) {
    const declarations = elementDeclarations(element, styleSource);
    return (/\bmin-width\s*:\s*0(?:px|rem|em|%)?\b/iu.test(declarations) ||
        /\boverflow(?:-x)?\s*:\s*(?:hidden|clip|auto|scroll)\b/iu.test(declarations));
}
function canvasParentWidthRemeasureEvidence(sources, canvas) {
    for (const source of sources) {
        const memberExpression = String.raw `[A-Za-z_$][\w$]*(?:\s*\.\s*[A-Za-z_$][\w$]*)*`;
        for (const displayWidth of source.content.matchAll(new RegExp(String.raw `\b(${memberExpression})\s*\.\s*style\s*\.\s*width\s*=\s*([^;\n]+)`, "gu"))) {
            const assignmentIndex = displayWidth.index ?? 0;
            const canvasVariable = (displayWidth[1] ?? "").replace(/\s+/gu, "");
            const canvasId = canvasVariableId(source.content.slice(0, assignmentIndex), canvasVariable) ??
                canvasIdsPassedToScalingFunction(source.content, assignmentIndex, canvasVariable)[0];
            if (!canvasVariable || !canvas.id || canvasId !== canvas.id)
                continue;
            const expression = displayWidth[2] ?? "";
            if (!expressionRemeasuresCanvasParent(source.content, assignmentIndex, canvasVariable, expression))
                continue;
            ...
            if (!new RegExp(`\\b${escapedCanvas}\\s*\\.\\s*width\\s*=`, "u").test(scopeBefore))
                continue;
            return sourceEvidence(source.path, source.lineOffset + lineNumberAt(source.content, assignmentIndex), sourceLineAt(source.content, assignmentIndex));
        }
    }
    return undefined;
}
function expressionRemeasuresCanvasParent(source, assignmentIndex, canvasVariable, expression) {
    const normalizedExpression = expression.replace(/\s+/gu, "");
    const escapedCanvas = canvasVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    if (new RegExp(`\\b${escapedCanvas}\\.(?:parentElement|parentNode)\\.(?:clientWidth|offsetWidth)\\b`, "u").test(normalizedExpression) ||
        new RegExp(`\\b${escapedCanvas}\\.(?:parentElement|parentNode)\\.getBoundingClientRect\\(\\)\\.width\\b`, "u").test(normalizedExpression)) {
        return true;
    }
    ...
    for (const alias of scopeBefore.matchAll(new RegExp(`\\b(?:const|let|var)\\s+([A-Za-z_$][\\w$]*)\\s*=\\s*${escapedCanvas}\\.(?:parentElement|parentNode)\\b`, "gu"))) {
        const name = alias[1];
        if (!name)
            continue;
        const escapedName = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        if (new RegExp(`\\b${escapedName}\\.(?:clientWidth|offsetWidth)\\b`, "u").test(normalizedExpression) ||
...
function responsivePaginationWidthIssues(html, scriptSources, stylesheets) {
    const styleSource = combinedStyleSource(html, stylesheets);
    const paginationElements = staticMarkupElements(html).filter((element) => /(?:pagination|paginator|pager|page[-_]?nav|分页)/iu.test(`${element.id} ${element.classNames.join(" ")}`));
    const warnings = [];
    for (const pagination of paginationElements) {
        const declarations = elementDeclarations(pagination, styleSource);
        if (!/\bdisplay\s*:\s*(?:inline-)?flex\b/iu.test(declarations))
            continue;
        if (/\bflex-wrap\s*:\s*wrap\b/iu.test(declarations) ||
        ...
        if (!evidence && staticButtons < 8)
            continue;
        warnings.push(`static.page_horizontal_overflow: Pagination renders every page control in one non-wrapping Flex row. Static source alone does not prove overflow at the 1440x900 desktop acceptance viewport, so this remains advisory and needs browser review. Target: ${markupElementLabel(pagination)}. Evidence: ${evidence ?? `index.html ${markupElementLabel(pagination)} contains ${staticButtons} buttons without flex-wrap.`}`);
    }
    return { errors: [], warnings: unique(warnings) };
}
function paginationAllPagesLoopEvidence(sources, pagination) {
    for (const source of sources) {
        const identityTokens = [pagination.id, ...pagination.classNames].filter(Boolean);
        if (identityTokens.length > 0 && !identityTokens.some((token) => source.content.includes(token)))
            continue;
        for (const loop of source.content.matchAll(/\bfor\s*\([^)]*(?:<=|<)\s*(?:totalPages|pageCount|numberOfPages|numPages)\b[^)]*\)\s*\{[\s\S]{0,3072}?\b(?:appendChild|append)\s*\(/giu)) {
            return sourceEvidence(source.path, source.lineOffset + lineNumberAt(source.content, loop.index ?? 0), sourceLineAt(source.content, loop.index ?? 0));
        }
    }
    return undefined;
}
function hasExplicitTableWidthPressure(table, styleSource, headerCount) {
    const tableDeclarations = elementDeclarations(table, styleSource);
    if (/\b(?:min-)?width\s*:\s*(?:max-content|min-content|fit-content)\b/iu.test(tableDeclarations) ||
    ...
    if (tableMinWidth !== undefined && tableMinWidth >= 480)
        return true;
...
function numericPixelDeclaration(declarations, property) {
...
function couldContainIntrinsicWidthViaExternalStylesheet(html, gridChild, table) {
    if (!hasExternalStylesheet(html))
        return false;
    let current = table.parent;
    while (current) {
        if (current.classNames.some((className) => /(?:^|[-_])(?:table[-_]?)?(?:responsive|scroll|overflow)(?:$|[-_])/iu.test(className))) {
            return true;
        }
        if (current === gridChild)
            break;
        current = current.parent;
    }
    return false;
}
function hasExplicitMultiColumnGrid(declarations) {
    for (const match of declarations.matchAll(/\bgrid-template-columns\s*:\s*([^;]+)/giu)) {
        const value = (match[1] ?? "").trim();
        const repeatCount = Number.parseInt(/\brepeat\(\s*(\d+)\s*,/iu.exec(value)?.[1] ?? "0", 10);
        if (repeatCount >= 2)
            return true;
        if (/\brepeat\(\s*(?:auto-fit|auto-fill)\s*,/iu.test(value))
            continue;
        const columns = value
            .replace(/\([^()]*(?:\([^()]*\)[^()]*)*\)/gu, "")
            .trim()
...
function containsIntrinsicWidth(gridChild, table, styleSource) {
    if (/\bmin-width\s*:\s*0(?:px|rem|em|%)?\b/iu.test(elementDeclarations(gridChild, styleSource)))
        return true;
    let current = table.parent;
    while (current) {
...
function markupElementLabel(element) {
    return [element.tagName, element.id ? `#${element.id}` : "", ...element.classNames.map((name) => `.${name}`)]
        .filter(Boolean)
        .join("");
}
function gridRuleEvidence(grid, html, stylesheets) {
    for (const source of [
        { path: "index.html", content: html },
        ...stylesheets.map(({ href, content }) => ({ path: href, content })),
    ]) {
        for (const match of source.content.matchAll(CSS_RULE_PATTERN)) {
            if (!/\bgrid-template-columns\s*:/iu.test(match[2] ?? ""))
                continue;
            if (!(match[1] ?? "").split(",").some((selector) => cssSelectorMatchesElement(selector, grid)))
                continue;
            return sourceEvidence(source.path, lineNumberAt(source.content, match.index ?? 0), (match[0] ?? "").replace(/\s+/gu, " "));
        }
    }
    return `index.html ${markupElementLabel(grid)} uses an explicit multi-column grid.`;
}
function couldBeBoundedByExternalStylesheet(canvas, html) {
    if (!hasExternalStylesheet(html) || !canvas.parent)
        return false;
    const parent = canvas.parent;
    if (CANVAS_CARD_SURFACE_CLASS_PATTERN.test(parent.classNames.join(" ")))
        return false;
    if (parent.children.length === 0 || parent.children.some((child) => !isCanvasOrViewportOverlay(child)))
        return false;
    return CHART_SEMANTIC_PATTERN.test(`${canvas.id} ${canvas.classNames.join(" ")} ${parent.id} ${parent.classNames.join(" ")}`);
}
function hasExternalStylesheet(html) {
    for (const match of html.matchAll(LINK_TAG_PATTERN)) {
        const attrs = match[1] ?? "";
        ...
        if ((!rel || rel.includes("stylesheet")) && reference?.kind === "external")
            return true;
    }
    return false;
}
function isNativeChartCanvas(canvas, html, drawingCallCount, hasResponsiveMeasurement, hasBitmapScaling, canvasCount) {
    ...
    if (fixedNumericSize && GAME_OR_DRAWING_CANVAS_PATTERN.test(semanticContext))
        return false;
    if (canvasCount > 1 && fixedNumericSize && !hasBitmapScaling)
        return false;
    return ((semantic && (drawing || hasResponsiveMeasurement || hasBitmapScaling)) ||
        (canvasCount === 1 && hasResponsiveMeasurement && drawing && hasBitmapScaling));
}
function canvasHasFixedNumericSize(html, id) {
    if (!id)
        return false;
    const escaped = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
function canvasMarkupNeighborhood(html, id) {
    if (!id)
        return "";
    const escaped = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
function canvasBitmapScalings(sources) {
    const scalings = [];
    for (const source of sources) {
        const memberExpression = String.raw `[A-Za-z_$][\w$]*(?:\s*\.\s*[A-Za-z_$][\w$]*)*`;
        ...
        const heightAssignments = [
            ...source.content.matchAll(new RegExp(String.raw `\b(${memberExpression})\s*\.\s*height\s*=\s*([^;\n]+)`, "gu")),
        ];
        for (const width of widthAssignments) {
            ...
            const height = heightAssignments
                .filter((candidate) => (candidate[1] ?? "").replace(/\s+/gu, "") === canvasVariable &&
                isScaledBitmapExpression(candidate[2] ?? "") &&
                Math.abs((candidate.index ?? 0) - (width.index ?? 0)) <= 4096)
                .sort((left, right) => Math.abs((left.index ?? 0) - (width.index ?? 0)) - Math.abs((right.index ?? 0) - (width.index ?? 0)))[0];
            if (!canvasVariable || !height || !isScaledBitmapExpression(widthExpression))
                continue;
...
function canvasHasScopedDisplaySize(source, assignmentIndex, canvasVariable) {
    const escapedVariable = canvasVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const range = enclosingJavaScriptFunctionRange(source, assignmentIndex);
    const scopedSource = range ? source.slice(range.start, range.end) : source;
    return (new RegExp(`\\b${escapedVariable}\\.style\\.width\\s*=`, "u").test(scopedSource) &&
        new RegExp(`\\b${escapedVariable}\\.style\\.height\\s*=`, "u").test(scopedSource));
}
function enclosingJavaScriptFunctionRange(source, index) {
    const declarations = [
        ...source.matchAll(/\bfunction\s+[A-Za-z_$][\w$]*\s*\([^)]*\)\s*\{/gu),
        ...source.matchAll(/\b(?:const|let|var)\s+[A-Za-z_$][\w$]*\s*=\s*(?:async\s*)?(?:\([^)]*\)|[A-Za-z_$][\w$]*)\s*=>\s*\{/gu),
    ...
    for (const declaration of declarations) {
        ...
        if (start < 0 || end < index || start >= index)
            continue;
...
function canvasIdsPassedToScalingFunction(source, assignmentIndex, canvasVariable) {
    ...
    for (const call of source.matchAll(new RegExp(`\\b${escapedName}\\s*\\(`, "gu"))) {
        ...
        if (openParenthesis === scope.openParenthesis)
            continue;
        ...
        const argument = source
            ...
            .split(/\s*,\s*/u)[scope.parameterIndex]?.trim();
        if (!argument)
            continue;
...
function enclosingCanvasFunction(source, assignmentIndex, canvasVariable) {
    for (const declaration of source.matchAll(/\bfunction\s+([A-Za-z_$][\w$]*)\s*\(([^)]*)\)\s*\{/gu)) {
        ...
        if (assignmentIndex <= openBrace || closeBrace < assignmentIndex)
            continue;
...
function hasResponsiveCanvasBitmapAspectMismatch(canvas, scaling, styleSource) {
...
function fixedScaledLogicalDimension(expression) {
...
function cssDimensionIsFullSize(declarations, property) {
    return new RegExp(`(?:^|;)\\s*${property}\\s*:\\s*100%\\s*(?:!important\\s*)?(?:;|$)`, "iu").test(declarations);
}
function hasExplicitSquareViewport(declarations) {
    if (/(?:^|;)\s*aspect-ratio\s*:\s*1(?:\s*\/\s*1)?\s*(?:;|$)/iu.test(declarations))
        return true;
...
function canvasVariableId(sourceBeforeAssignment, canvasVariable) {
    const escapedVariable = canvasVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const bindings = [
        ...sourceBeforeAssignment.matchAll(new RegExp(String.raw `\b(?:const|let|var)\s+${escapedVariable}\s*=\s*document\.getElementById\(\s*(['"])([^'"]+)\1\s*\)`, "gu")),
        ...sourceBeforeAssignment.matchAll(new RegExp(String.raw `\b(?:const|let|var)\s+${escapedVariable}\s*=\s*document\.querySelector\(\s*(['"])#([^'"]+)\1\s*\)`, "gu")),
        ...sourceBeforeAssignment.matchAll(new RegExp(String.raw `\b${escapedVariable}\s*=\s*document\.getElementById\(\s*(['"])([^'"]+)\1\s*\)`, "gu")),
        ...sourceBeforeAssignment.matchAll(new RegExp(String.raw `\b${escapedVariable}\s*=\s*document\.querySelector\(\s*(['"])#([^'"]+)\1\s*\)`, "gu")),
    ];
    const member = /^([A-Za-z_$][\w$]*)\.([A-Za-z_$][\w$]*)$/u.exec(canvasVariable);
    if (member) {
        const root = member[1]?.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") ?? "";
        const property = member[2]?.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") ?? "";
        bindings.push(...sourceBeforeAssignment.matchAll(new RegExp(String.raw `\b(?:const|let|var)\s+${root}\s*=\s*\{[\s\S]{0,16384}?\b${property}\s*:\s*document\.getElementById\(\s*(['"])([^'"]+)\1\s*\)`, "gu")), ...sourceBeforeAssignment.matchAll(new RegExp(String.raw `\b(?:const|let|var)\s+${root}\s*=\s*\{[\s\S]{0,16384}?\b${property}\s*:\s*document\.querySelector\(\s*(['"])#([^'"]+)\1\s*\)`, "gu")));
    }
    bindings.sort((left, right) => (left.index ?? 0) - (right.index ?? 0));
    return bindings.at(-1)?.[2];
}
function isScaledBitmapExpression(expression) {
    return (/\b(?:devicePixelRatio|dpr|pixelRatio|backingStoreRatio)\b/iu.test(expression) ||
        /(?:\*\s*(?:[2-9]|\d{2,})(?:\.\d+)?\b|\b(?:[2-9]|\d{2,})(?:\.\d+)?\s*\*)/u.test(expression));
}
function hasExplicitCanvasDisplaySize(canvas, styleSource, scaling) {
    return scaling.hasScopedDisplaySize || hasCssDisplayDimensions(elementDeclarations(canvas, styleSource));
}
function hasCssDisplayDimensions(declarations) {
    return (/(?:^|;)\s*width\s*:\s*(?!(?:auto|initial|inherit|unset)\b)[^;]+/iu.test(declarations) &&
        /(?:^|;)\s*height\s*:\s*(?!(?:auto|initial|inherit|unset)\b)[^;]+/iu.test(declarations));
}
function isDedicatedBoundedCanvasViewport(parent, styleSource) {
    if (CANVAS_CARD_SURFACE_CLASS_PATTERN.test(parent.classNames.join(" ")))
        return false;
    if (parent.children.some((child) => !isCanvasOrViewportOverlay(child)))
        return false;
    return hasBoundedChartSize(elementDeclarations(parent, styleSource), styleSource);
}
function isDedicatedBoundedChartJsViewport(parent, styleSource) {
    ...
    const containsCanvasOnly = parent.children.length > 0 && parent.children.every((child) => child.tagName === "canvas");
    const semanticContainer = DEDICATED_CHART_CONTAINER_SELECTOR_PATTERN.test(`${parent.id} ${className}`) ||
...
function isCanvasOrViewportOverlay(child) {
...
function hasBoundedChartSize(declarations, styleSource) {
    if (BOUNDED_CHART_SIZE_PATTERN.test(declarations))
        return true;
    for (const match of declarations.matchAll(BOUNDED_CHART_SIZE_VARIABLE_PATTERN)) {
        ...
        if (definition && BOUNDED_CHART_SIZE_PATTERN.test(`height:${definition}`))
            return true;
    }
    return false;
}
function elementDeclarations(element, styleSource) {
    const declarations = [];
    for (const match of styleSource.matchAll(CSS_RULE_PATTERN)) {
        for (const selector of (match[1] ?? "").split(",")) {
            if (cssSelectorMatchesElement(selector, element))
...
function cssSelectorMatchesElement(selector, element) {
    ...
    if (!classes.every((className) => element.classNames.includes(className)))
        return false;
    return Boolean(tag || id || classes.length > 0);
}
function invalidViewportEvidence(canvas) {
    if (!canvas)
        return "index.html contains a chart canvas without a measurable viewport.";
    const parent = canvas.parent;
    if (!parent)
        return `index.html canvas ${chartCanvasLabel(canvas)} has no parent viewport.`;
    const parentLabel = [
        parent.tagName,
        parent.id ? `#${parent.id}` : "",
...
function chartCanvasLabels(canvases) {
    return canvases.map(chartCanvasLabel).join(", ");
}
function chartCanvasLabel(canvas) {
    return canvas.id
        ? `#${canvas.id}`
...
function staticCanvasElements(html) {
    return staticMarkupElements(html).filter((element) => element.tagName === "canvas");
}
function staticMarkupElements(html) {
    ...
    for (const match of masked.matchAll(tagPattern)) {
        ...
        if (!VOID_HTML_TAGS.has(tagName) && !token.endsWith("/>"))
            stack.push(element);
    }
    return elements;
}
function maskHtmlBlocks(html, patterns) {
    let masked = html;
    for (const pattern of patterns)
        masked = masked.replace(pattern, (match) => " ".repeat(match.length));
    return masked;
}
function combinedStyleSource(html, stylesheets) {
    return [
        ...[...html.matchAll(STYLE_TAG_PATTERN)].map((match) => match[1] ?? ""),
        ...stylesheets.map((stylesheet) => stylesheet.content),
    ].join("\n");
}
function collectScriptSources(html, scripts) {
    ...
    for (const match of html.matchAll(SCRIPT_BLOCK_PATTERN)) {
        if (staticHtmlAttributeValue(match[1] ?? "", "src"))
            continue;
...
function nondeterministicRenderedDataEvidence(sources, dataDrivenPage) {
    for (const source of sources) {
        ...
        for (const match of executableSource.matchAll(MOCK_DATA_RANDOM_PATTERN)) {
            ...
            if (!name || randomOffset < 0 || invocationCount(executableSource, name) < minimumOccurrences)
                continue;
            return {
        ...
        for (const match of executableSource.matchAll(OBJECT_DATA_METHOD_RANDOM_PATTERN)) {
            const name = match[1] ?? "";
            const randomOffset = match[0].lastIndexOf("Math.random");
            if (!name || randomOffset < 0 || invocationCount(executableSource, name) < 1)
                continue;
            return {
                evidence: evidenceAt(source, (match.index ?? 0) + randomOffset),
                highConfidence: true,
        ...
        for (const declaration of executableSource.matchAll(OBJECT_DATA_METHOD_SHORTHAND_PATTERN)) {
            ...
            if (!name || closeBrace < 0 || invocationCount(executableSource, name) < 2)
                continue;
...
function transitiveObjectRandomDataIndex(source) {
    ...
    for (const declaration of declarations) {
        ...
        const openBrace = (declaration.index ?? 0) + declaration[0].lastIndexOf("{");
        const closeBrace = matchingJavaScriptBrace(source, openBrace);
        if (!objectName || closeBrace < 0 || closeBrace - openBrace > 65_536)
            continue;
        const objectBody = source.slice(openBrace + 1, closeBrace);
        const methodDeclarations = [
            ...objectBody.matchAll(/(?:^|,)\s*([A-Za-z_$][\w$]*)\s*\([^)]*\)\s*\{/gu),
            ...objectBody.matchAll(/(?:^|,)\s*([A-Za-z_$][\w$]*)\s*:\s*(?:function\s*\([^)]*\)|(?:\([^)]*\)|[A-Za-z_$][\w$]*)\s*=>)\s*\{/gu),
        ].sort((left, right) => (left.index ?? 0) - (right.index ?? 0));
        const methods = [];
        for (const method of methodDeclarations) {
            const name = method[1] ?? "";
            if (!name || /^(?:if|for|while|switch|catch|function)$/u.test(name))
                continue;
            ...
            if (methodCloseBrace < 0 || methodCloseBrace > closeBrace)
                continue;
        ...
        const tainted = new Set(methods.filter((method) => method.randomIndex !== undefined).map((method) => method.name));
        if (tainted.size === 0)
            continue;
        ...
        for (let pass = 0; pass < methods.length; pass += 1) {
            ...
            for (const method of methods) {
                if (tainted.has(method.name))
                    continue;
                const callsTaintedMethod = [...tainted].some((callee) => {
                    const escapedCallee = callee.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                    return new RegExp(String.raw `\b(?:this|${escapedObjectName})\s*\.\s*${escapedCallee}\s*\(`, "u").test(method.body);
                });
                if (!callsTaintedMethod)
                    continue;
        ...
        if (!invokedTaintedMethod)
            continue;
        const randomSource = methods.find((method) => method.randomIndex !== undefined && tainted.has(method.name));
        if (randomSource?.randomIndex !== undefined)
            return randomSource.randomIndex;
    }
    return undefined;
}
function declaredFunctionInvocationMinimum(source, name) {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
function firstSourceEvidence(sources, pattern) {
    for (const source of sources) {
        const match = pattern.exec(source.content);
        if (match?.index !== undefined)
            return evidenceAt(source, match.index);
    }
    return undefined;
}
function evidenceAt(source, index) {
    return sourceEvidence(source.path, source.lineOffset + lineNumberAt(source.content, index), sourceLineAt(source.content, index));
}
function sourceEvidence(path, line, excerpt) {
...
function unwiredSelectErrors(html, scriptSource) {
    if (GENERIC_SELECT_VALUE_HANDLER_PATTERN.test(scriptSource) ||
...
function delegatedContainerSelectHandlerReferencesId(html, source, id) {
    ...
    for (let ancestor = select.parent; ancestor; ancestor = ancestor.parent) {
        ...
        const listenerPattern = new RegExp(String.raw `(?:getElementById\(\s*(['"\x60])${escapedAncestor}\1\s*\)|querySelector\(\s*(['"\x60])#${escapedAncestor}\2\s*\))\s*\.\s*addEventListener\(\s*(['"\x60])change\3\s*,\s*(?:\(\s*)?([A-Za-z_$][\w$]*)(?:\s*\))?\s*=>\s*\{`, "gu");
        for (const listener of source.matchAll(listenerPattern)) {
            ...
            const closeBrace = matchingJavaScriptBrace(source, openBrace);
            if (!eventVariable || closeBrace < 0)
                continue;
            const escapedEvent = eventVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            const body = source.slice(openBrace + 1, closeBrace);
            if (new RegExp(`\\b${escapedEvent}\\s*\\.\\s*target\\s*\\.\\s*id\\b`, "u").test(body) &&
                new RegExp(`\\b${escapedEvent}\\s*\\.\\s*target\\s*\\.\\s*value\\b`, "u").test(body)) {
                return true;
            }
        }
        const expressionListenerPattern = new RegExp(String.raw `(?:getElementById\(\s*(['"\x60])${escapedAncestor}\1\s*\)|querySelector\(\s*(['"\x60])#${escapedAncestor}\2\s*\))\s*\.\s*addEventListener\(\s*(['"\x60])change\3\s*,\s*(?:\(\s*)?([A-Za-z_$][\w$]*)(?:\s*\))?\s*=>\s*([^;\r\n]{1,1024})`, "gu");
...
function explicitSelectBindingMapReferencesId(source, id) {
    return explicitSelectBindingMaps(source).some((bindingMap) => bindingMap.entries.some((entry) => entry.id === id));
}
function explicitSelectBindingMaps(source) {
    const maps = [];
    for (const declaration of source.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*\[([\s\S]{1,8192}?)\]\s*;/gu)) {
        ...
        const loop = new RegExp(String.raw `\b${escapedCollection}\s*\.\s*forEach\s*\(\s*([A-Za-z_$][\w$]*)\s*=>\s*\{`, "gu").exec(source);
        ...
        if (!parameter || closeBrace < 0)
            continue;
        const escapedParameter = parameter.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const body = source.slice(openBrace + 1, closeBrace);
        if (!new RegExp(String.raw `getElementById\s*\(\s*${escapedParameter}\s*\.\s*id\s*\)`, "u").test(body) ||
            !/\.\s*addEventListener\s*\(\s*(['"`])change\1/u.test(body) ||
            !new RegExp(String.raw `\bstate\s*\.\s*filters\s*\[\s*${escapedParameter}\s*\.\s*(?:key|property|stateKey)\s*\]\s*=\s*[A-Za-z_$][\w$]*\s*\.\s*target\s*\.\s*value\b`, "iu").test(body)) {
            continue;
        }
...
function dynamicConstructedSelectHandlerReferencesId(source, id) {
    const loops = [];
    ...
    for (const declaration of source.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*\[([\s\S]{1,1024}?)\]\s*;/gu)) {
        const collection = declaration[1];
        if (!collection)
            continue;
        const escapedCollection = collection.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        for (const loop of source.matchAll(new RegExp(String.raw `\b${escapedCollection}\s*\.\s*forEach\s*\(\s*([A-Za-z_$][\w$]*)\s*=>\s*\{`, "gu"))) {
            const parameter = loop[1];
            const openBrace = (loop.index ?? 0) + loop[0].lastIndexOf("{");
            const closeBrace = matchingJavaScriptBrace(source, openBrace);
            if (!parameter || closeBrace < 0)
    ...
    for (const loop of loops) {
        ...
        if (!/\.\s*addEventListener\s*\(\s*(['"])change\1/u.test(body) || !/\.\s*value\b/u.test(body))
            continue;
        const escapedParameter = parameter.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        ...
        const prefixedConstruction = new RegExp(String.raw `getElementById\(\s*(['"\x60])([^'"\x60$]*)\1\s*\+\s*${escapedParameter}\s*\)`, "u").exec(body);
        const prefix = prefixedConstruction?.[2];
        if (prefix !== undefined && id.startsWith(prefix)) {
            const item = id.slice(prefix.length).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            if (new RegExp(`(['"])${item}\\1`, "u").test(loop.items))
                return true;
        }
        const suffixedConstruction = new RegExp(String.raw `getElementById\(\s*${escapedParameter}\s*\+\s*(['"\x60])([^'"\x60$]*)\1\s*\)`, "u").exec(body);
...
function delegatedSelectHandlerReferencesId(source, id) {
    const escapedId = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    for (const listener of source.matchAll(/\.\s*addEventListener\(\s*(['"])change\1\s*,\s*(?:\(\s*)?([A-Za-z_$][\w$]*)(?:\s*\))?\s*=>\s*\{/gu)) {
        ...
        if (!new RegExp(`\\b${escapedEvent}\\s*\\.\\s*target\\s*\\.\\s*id\\b`, "u").test(body))
            continue;
        if (!new RegExp(`\\b${escapedEvent}\\s*\\.\\s*target\\s*\\.\\s*value\\b`, "u").test(body))
            continue;
        if (new RegExp(`(['"])${escapedId}\\1`, "u").test(body))
            return true;
    }
    return false;
}
function unusedSelectValueErrors(html, sources) {
        ...
        if (resetOnlyMatches.length > 0 &&
            ...
            if (![...sourceWithoutResetAssignments.matchAll(idReferencePattern)].length) {
                for (const source of sources) {
                    ...
                    if (resetMatch?.index === undefined)
                        continue;
                    errors.push(`static.filter_value_unused: Select #${id} is only reset and is never read by an applicable filter handler. Evidence: ${evidenceAt(source, resetMatch.index)}`);
                    break;
                }
                continue;
            }
        ...
        if (referenceCount === 1 &&
            ...
            for (const source of sources) {
                ...
                if (reference?.index === undefined)
                    continue;
                errors.push(`static.filter_value_unused: Select #${id} is bound through a generic change handler, but its value is never read by rendered-data code. Evidence: ${evidenceAt(source, reference.index)}`);
                break;
            }
            continue;
        }
        const capturePattern = new RegExp(String.raw `\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:(?:document\.)?(?:getElementById|querySelector)\(\s*['"\x60]#?${escapedId}['"\x60]\s*\)|\$\(\s*['"\x60]#${escapedId}['"\x60]\s*\))\.value\b`, "u");
        for (const source of sources) {
            const capture = capturePattern.exec(source.content);
            const variable = capture?.[1];
            if (!variable || capture?.index === undefined)
                continue;
            const escapedVariable = variable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            if ([...combined.matchAll(new RegExp(`\\b${escapedVariable}\\b`, "gu"))].length !== 1)
                continue;
            errors.push(`static.filter_value_unused: Select #${id} reads its value into ${variable} but never uses that value. Evidence: ${evidenceAt(source, capture.index)}`);
        }
    }
    return unique(errors);
}
function inertDelegatedFilterStateErrors(html, sources) {
    ...
    for (const source of sources) {
        ...
        if (/\.\s*subscribe\s*\(\s*(?:\(\s*[A-Za-z_$][\w$]*|[A-Za-z_$][\w$]*\s*=>|[A-Za-z_$][\w$]*\s*\))/u.test(source.content)) {
            continue;
        }
        ...
        for (const selectMatch of html.matchAll(SELECT_TAG_PATTERN)) {
            ...
            errors.push(`static.filter_value_unused: Select #${id} updates this.filters.${property}, but getFilteredData() returns this.dataset unchanged and the redraw subscriber does not consume filter arguments. Evidence: ${evidence}`);
        }
    }
    return unique(errors);
}
function explicitBindingMapFilterErrors(html, sources) {
    const errors = [];
    const combined = sources.map((source) => source.content).join("\n");
    ...
    for (const source of sources) {
        for (const bindingMap of explicitSelectBindingMaps(source.content)) {
            ...
            for (const entry of bindingMap.entries) {
                if (!selectIdExists(html, entry.id) || !declaredFilterProperties.has(entry.property))
                    continue;
                if (renderedDataReadsStateFilterProperty(combined, entry.property))
                    continue;
                errors.push(`static.filter_value_unused: Select #${entry.id} writes state.filters.${entry.property} through an explicit binding map, but rendered-data code never reads that filter property. Evidence: ${evidenceAt(source, entry.index)}`);
            }
            const mappedElements = markupSelects.filter((element) => mappedIds.has(element.id));
            const mappedParentCounts = new Map();
            for (const element of mappedElements) {
                for (let ancestor = element.parent; ancestor; ancestor = ancestor.parent) {
                    if (!/(?:filter|control)/iu.test(`${ancestor.id} ${ancestor.classNames.join(" ")}`))
                        continue;
            ...
            for (const [parent, count] of mappedParentCounts) {
                ...
                for (const select of descendants(parent).filter((child) => child.tagName === "select")) {
                    if (!select.id || mappedIds.has(select.id))
                        continue;
                    const property = kebabToCamelCase(select.id.replace(/^filter-/iu, "").replace(/-filter$/iu, ""));
                    if (!property || !declaredFilterProperties.has(property))
                        continue;
                    if (scriptReferencesId(combined, select.id) ||
                        delegatedContainerSelectHandlerReferencesId(html, combined, select.id)) {
                        continue;
                    }
                    errors.push(`static.filter_value_unused: Select #${select.id} is omitted from the explicit filter binding map and cannot update declared state.filters.${property}. Evidence: ${evidenceAt(source, bindingMap.index)}`);
                }
            }
        }
    }
...
function objectMethodBody(source, name) {
    const escapedName = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const patterns = [
...
function delegatedUpdateFilterCall(source) {
    for (const listener of source.matchAll(/\.\s*addEventListener\s*\(\s*(['"`])change\1\s*,\s*(?:\(\s*)?([A-Za-z_$][\w$]*)(?:\s*\))?\s*=>\s*\{/gu)) {
        const eventVariable = listener[2];
        const openBrace = (listener.index ?? 0) + listener[0].lastIndexOf("{");
        const closeBrace = matchingJavaScriptBrace(source, openBrace);
        if (!eventVariable || closeBrace < 0)
        ...
        const escapedEvent = eventVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const body = source.slice(openBrace + 1, closeBrace);
        if (/\.\s*updateFilter\s*\(/u.test(body) &&
...
function objectPropertyKeys(source, propertyName) {
    ...
    if (closeBrace < 0)
        return new Set();
    const body = source.slice(openBrace + 1, closeBrace);
    return new Set([...body.matchAll(/(?:^|,)\s*([A-Za-z_$][\w$]*)\s*:/gu)].map((match) => match[1] ?? "").filter(Boolean));
}
function allObjectPropertyKeys(source, propertyName) {
    const keys = new Set();
    const escapedName = propertyName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
function enabledStaticSelectOptionCount(selectHtml) {
    return [...selectHtml.matchAll(/<option\b([^>]*)>/giu)].filter((option) => !/(?:^|\s)disabled(?:\s|=|$)/iu.test(option[1] ?? "")).length;
}
...
export function executableMathRandomCallIndex(source) {
...
function javascriptRegexLiteralEnd(characters, start) {
    let previous = start - 1;
    while (previous >= 0 && /\s/u.test(characters[previous] ?? ""))
        previous -= 1;
    const previousCharacter = previous >= 0 ? (characters[previous] ?? "") : "";
    const prefixAllowsRegex = previous < 0 ||
        /[=(:,!&|?{;[]/u.test(previousCharacter) ||
        (previousCharacter === ">" && characters[previous - 1] === "=") ||
        /\b(?:return|case|throw|delete|void|typeof|instanceof|in|of|yield|await)\s*$/u.test(characters.slice(0, start).join(""));
    if (!prefixAllowsRegex)
        return -1;
    ...
    for (let index = start + 1; index < characters.length; index += 1) {
        ...
        if (current !== "/" || inCharacterClass)
            continue;
        while (/[A-Za-z]/u.test(characters[index + 1] ?? ""))
            index += 1;
        return index;
    }
    return -1;
}
function deadDynamicConstructedFilterPropertyErrors(sources) {
    const errors = [];
    const combined = sources.map((source) => source.content).join("\n");
    ...
    if (/\bstate\s*\.\s*filters\s*\[/u.test(withoutComputedAssignments) ||
        /\bstate\s*\.\s*filters\b(?!\s*\.)/u.test(withoutComputedAssignments)) {
        return errors;
    }
    if (!computedAssignmentPattern.test(combined))
        return errors;
    for (const source of sources) {
        for (const definition of source.content.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*\[([\s\S]{1,8192}?)\]\s*;/gu)) {
            const collectionName = definition[1];
            const items = definition[2] ?? "";
            if (!collectionName || !/(?:filter|control|selector)/iu.test(collectionName))
                continue;
            const escapedCollection = collectionName.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
            const map = new RegExp(String.raw `\b${escapedCollection}\s*\.\s*map\s*\(\s*([A-Za-z_$][\w$]*)\s*=>[\s\S]{0,4096}?<select\b[^>]*\bid\s*=\s*(["'\x60])([^"'\x60$]*)\$\{\s*\1\s*\.\s*id\s*\}`, "u").exec(source.content);
            const prefix = map?.[3];
            if (prefix === undefined)
                continue;
            ...
            for (const idMatch of ids) {
                ...
                if (!property || renderedDataReadsStateFilterProperty(withoutComputedAssignments, property))
                    continue;
                const itemIndex = (definition.index ?? 0) + definition[0].indexOf(items) + (idMatch.index ?? 0);
                errors.push(`static.filter_value_unused: Select #${prefix}${property} writes state.filters.${property} through a dynamic filter definition, but rendered-data code never reads that filter property. Evidence: ${evidenceAt(source, itemIndex)}`);
            }
        }
    }
    return errors;
}
function advisoryOnlySelectHandlerErrors(sources) {
    const errors = [];
    for (const source of sources) {
        for (const handler of declaredFunctionBodies(source.content)) {
            if (!/(?:apply|filter|search|query|refresh)/iu.test(handler.name))
                continue;
            const captures = [
                ...handler.body.matchAll(/\b(?:const|let|var)\s+[A-Za-z_$][\w$]*\s*=\s*(?:document\.)?(?:getElementById|querySelector)\(\s*(['"`])#?([^'"`$]+)\1\s*\)\s*\.\s*value\b[^;]*;?/gu),
            ];
            if (captures.length === 0)
                continue;
            let residual = handler.body;
            for (const capture of captures)
                residual = residual.replace(capture[0], "");
            ...
            for (const capture of captures) {
                ...
                if (!id)
                    continue;
                errors.push(`static.filter_value_unused: Select #${id} is read only by advisory-only handler ${handler.name}, which logs or displays the value without changing rendered data. Evidence: ${evidenceAt(source, handler.index)}`);
            }
        }
    }
    return errors;
}
function placeholderOnlyFilterHandlerErrors(sources) {
    const errors = [];
    for (const source of sources) {
        for (const handler of declaredFunctionBodies(source.content)) {
            if (!/(?:apply|filter|refresh)/iu.test(handler.name))
                continue;
            if (!/(?:would\s+go\s+here|to-?do|not\s+implemented|just\s+(?:trigger|refresh|update)[\s\S]{0,120}(?:current|existing|same)\s+data)/iu.test(handler.body)) {
                continue;
            }
            const executableBody = handler.body.replace(/\/\*[\s\S]*?\*\//gu, "").replace(/\/\/[^\r\n]*/gu, "");
            if (/\bfilters?\b/iu.test(executableBody))
                continue;
            if (!/(?:\.update\s*\(|update(?:kpis?|charts?|metrics?|tables?)\s*\()/iu.test(executableBody))
                continue;
            errors.push(`static.filter_value_unused: Handler ${handler.name} explicitly leaves filtering unimplemented and only redraws existing data. Evidence: ${evidenceAt(source, handler.index)}`);
        }
    }
    return errors;
}
function deadBulkStateFilterPropertyErrors(sources) {
    const errors = [];
    const assignmentPattern = /\bstate\s*\.\s*filters\s*=\s*\{([\s\S]{0,4096}?)\}\s*;?/gu;
    const assignments = sources.flatMap((source) => [...source.content.matchAll(assignmentPattern)].map((assignment) => ({ source, assignment })));
    if (assignments.length === 0)
        return errors;
    const combinedWithoutAssignments = sources.map((source) => source.content.replace(assignmentPattern, "")).join("\n");
    if (/\bstate\s*\.\s*filters\s*\[/u.test(combinedWithoutAssignments) ||
        /\bstate\s*\.\s*filters\b(?!\s*\.)/u.test(combinedWithoutAssignments)) {
        return errors;
    }
    const entryPattern = /\b([A-Za-z_$][\w$]*)\s*:\s*(?:document\s*\.\s*)?(?:getElementById|querySelector)\(\s*(['"`])#?([^'"`$]+)\2\s*\)\s*\.\s*value\b/gu;
    for (const { source, assignment } of assignments) {
        ...
        for (const entry of objectBody.matchAll(entryPattern)) {
            ...
            if (new RegExp(String.raw `\bstate\s*\.\s*filters\s*\.\s*${escapedProperty}\b`, "u").test(combinedWithoutAssignments)) {
                continue;
            }
            const entryIndex = (assignment.index ?? 0) + assignment[0].indexOf(objectBody) + (entry.index ?? 0);
            errors.push(`static.filter_value_unused: Select #${id} writes state.filters.${property} in a bulk filter assignment, but rendered-data code never reads that filter property. Evidence: ${evidenceAt(source, entryIndex)}`);
        }
    }
    return errors;
}
function deadAliasedStateFilterPropertyErrors(sources) {
    const errors = [];
    const combined = sources.map((source) => source.content).join("\n");
    ...
    const elementAliasPattern = /\b([A-Za-z_$][\w$]*(?:\s*\.\s*[A-Za-z_$][\w$]*)*)\s*=\s*(?:document\s*\.\s*)?(?:getElementById|querySelector)\(\s*(['"`])#?([^'"`$]+)\2\s*\)/gu;
    for (const source of sources) {
        for (const alias of source.content.matchAll(elementAliasPattern)) {
            const qualifiedName = alias[1]?.replace(/\s+/gu, "");
            const id = alias[3];
            if (qualifiedName && id)
                elementIds.set(qualifiedName, id);
        }
    }
    if (elementIds.size === 0)
        return errors;
    const stateAssignmentPattern = /\bstate\s*\.\s*filters\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*([A-Za-z_$][\w$]*(?:\s*\.\s*[A-Za-z_$][\w$]*)*)\s*\.\s*value\b/gu;
    for (const source of sources) {
        for (const assignment of source.content.matchAll(stateAssignmentPattern)) {
            ...
            if (!property || !id || renderedDataReadsStateFilterProperty(combined, property))
                continue;
            errors.push(`static.filter_value_unused: Select #${id} writes state.filters.${property} through DOM alias ${alias}, but rendered-data code never reads that filter property. Evidence: ${evidenceAt(source, assignment.index ?? 0)}`);
        }
    }
    return errors;
}
function deadStateFilterPropertyErrors(sources) {
    const errors = [];
    const combined = sources.map((source) => source.content).join("\n");
    if (/\bstate\s*\.\s*filters\s*\[/u.test(combined))
        return errors;
    const bareStateFilterUse = /\bstate\s*\.\s*filters\b(?!\s*\.)/u.test(combined);
    if (bareStateFilterUse)
        return errors;
    const bindingPattern = /\b(?:document\s*\.\s*)?getElementById\(\s*(['"`])([^'"`$]+)\1\s*\)\s*\.\s*addEventListener\(\s*(['"`])change\3\s*,\s*(?:\(\s*)?([A-Za-z_$][\w$]*)(?:\s*\))?\s*=>\s*\{([\s\S]{0,768}?)\}\s*\)/gu;
    for (const source of sources) {
        for (const binding of source.content.matchAll(bindingPattern)) {
            ...
            if (!id || !eventVariable)
                continue;
            const escapedEvent = eventVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            const assignment = new RegExp(String.raw `\bstate\s*\.\s*filters\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*${escapedEvent}\s*\.\s*target\s*\.\s*value\b`, "u").exec(body);
            const property = assignment?.[1];
            if (!property)
                continue;
            if (renderedDataReadsStateFilterProperty(combined, property))
                continue;
            errors.push(`static.filter_value_unused: Select #${id} writes state.filters.${property}, but rendered-data code never reads that filter property. Evidence: ${evidenceAt(source, binding.index ?? 0)}`);
        }
    }
    return errors;
}
function renderedDataReadsStateFilterProperty(source, property) {
    const escapedProperty = property.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
function deadSharedStateFilterPropertyErrors(html, sources) {
    const errors = [];
    const combined = sources.map((source) => source.content).join("\n");
    ...
    const bindingPattern = /\b[A-Za-z_$][\w$]*\s*\.\s*addEventListener\(\s*(['"`])change\1\s*,\s*(?:\(\s*)?([A-Za-z_$][\w$]*)(?:\s*\))?\s*=>\s*\{/gu;
    for (const source of sources) {
        for (const binding of source.content.matchAll(bindingPattern)) {
            ...
            if (!eventVariable || openBrace < 0 || closeBrace < 0)
                continue;
            const body = source.content.slice(openBrace + 1, closeBrace);
            const escapedEvent = eventVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            const derivedKeys = new Map();
            for (const derived of body.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*[A-Za-z_$][\w$]*(?:\s*\.\s*[A-Za-z_$][\w$]*)*\s*\.\s*replace\(\s*(['"`])([^'"`$]*)\2\s*,\s*(['"`])\4\s*\)/gu)) {
                const keyVariable = derived[1];
                const removedPrefix = derived[3];
                if (keyVariable !== undefined && removedPrefix !== undefined)
                    derivedKeys.set(keyVariable, removedPrefix);
            }
            const branchPattern = new RegExp(String.raw `(?:if|else\s+if)\s*\(\s*([A-Za-z_$][\w$]*)\s*={2,3}\s*(['"\x60])([^'"\x60$]+)\2\s*\)\s*state\s*\.\s*filters\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*${escapedEvent}\s*\.\s*target\s*\.\s*value\b`, "gu");
            for (const branch of body.matchAll(branchPattern)) {
                ...
                if (!keyVariable || !comparedValue || !property)
                    continue;
                ...
                const id = selectIdExists(html, comparedValue)
                    ...
                    : selectIdExists(html, prefixedId)
                        ? prefixedId
                        : undefined;
                if (!id || renderedDataReadsStateFilterProperty(combined, property))
                    continue;
                errors.push(`static.filter_value_unused: Select #${id} writes state.filters.${property} through a shared change handler, but rendered-data code never reads that filter property. Evidence: ${evidenceAt(source, openBrace + 1 + (branch.index ?? 0))}`);
            }
        }
        ...
        for (const derived of source.content.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*[A-Za-z_$][\w$]*(?:\s*\.\s*[A-Za-z_$][\w$]*)*\s*\.\s*replace\(\s*(['"`])([^'"`$]*)\2\s*,\s*(['"`])\4\s*\)/gu)) {
            const keyVariable = derived[1];
            const removedPrefix = derived[3];
            const derivedIndex = derived.index ?? 0;
            if (!keyVariable || removedPrefix === undefined)
                continue;
            const boundedStart = Math.max(0, derivedIndex - 512);
            const boundedWindow = source.content.slice(boundedStart, derivedIndex + 4096);
            if (!/\.\s*addEventListener\(\s*(['"`])change\1/iu.test(boundedWindow))
                continue;
            for (const select of html.matchAll(SELECT_TAG_PATTERN)) {
                const id = attributeValue(select[1] ?? "", "id");
                if (!id || !id.startsWith(removedPrefix))
                    continue;
                ...
                const branch = new RegExp(String.raw `(?:if|else\s+if)\s*\(\s*${escapedKey}\s*={2,3}\s*(['"\x60])${escapedValue}\1\s*\)\s*state\s*\.\s*filters\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*[A-Za-z_$][\w$]*\s*\.\s*target\s*\.\s*value\b`, "u").exec(boundedWindow);
                const property = branch?.[2];
                if (!property || renderedDataReadsStateFilterProperty(combined, property))
                    continue;
                errors.push(`static.filter_value_unused: Select #${id} writes state.filters.${property} through a shared change handler, but rendered-data code never reads that filter property. Evidence: ${evidenceAt(source, boundedStart + (branch?.index ?? 0))}`);
            }
        }
        if (/\.\s*addEventListener\(\s*(['"`])change\1/iu.test(source.content)) {
            for (const branch of source.content.matchAll(/(?:if|else\s+if)\s*\(\s*([A-Za-z_$][\w$]*)\s*={2,3}\s*(['"`])([^'"`$]+)\2\s*\)\s*state\s*\.\s*filters\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*[A-Za-z_$][\w$]*\s*\.\s*target\s*\.\s*value\b/gu)) {
                const keyVariable = branch[1];
                const comparedValue = branch[3];
                const property = branch[4];
                if (!keyVariable || !comparedValue || !property)
                    continue;
                const escapedKey = keyVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                const prefixPattern = new RegExp(String.raw `\b(?:const|let|var)\s+${escapedKey}\s*=\s*[A-Za-z_$][\w$]*(?:\s*\.\s*[A-Za-z_$][\w$]*)*\s*\.\s*replace\(\s*(['"\x60])([^'"\x60$]*)\1\s*,\s*(['"\x60])\3\s*\)`, "gu");
                const branchIndex = branch.index ?? 0;
                const prefixes = [...source.content.matchAll(prefixPattern)].filter((candidate) => (candidate.index ?? 0) <= branchIndex && branchIndex - (candidate.index ?? 0) <= 4096);
                ...
                if (!id || renderedDataReadsStateFilterProperty(combined, property))
                    continue;
                errors.push(`static.filter_value_unused: Select #${id} writes state.filters.${property} through a shared change handler, but rendered-data code never reads that filter property. Evidence: ${evidenceAt(source, branchIndex)}`);
            }
        }
    }
    return unique(errors);
}
function deadFlatStateFilterPropertyErrors(html, sources) {
    const errors = [];
    const combined = sources.map((source) => source.content).join("\n");
    ...
    for (const source of sources) {
        ...
        for (const declaration of source.content.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*\{/gu)) {
            ...
            const openBrace = (declaration.index ?? 0) + declaration[0].lastIndexOf("{");
            const closeBrace = matchingJavaScriptBrace(source.content, openBrace);
            if (!objectName || openBrace < 0 || closeBrace < 0)
                continue;
            const objectBody = source.content.slice(openBrace + 1, closeBrace);
            for (const entry of objectBody.matchAll(/(?:^|,)\s*([A-Za-z_$][\w$]*)\s*:\s*(?:document\s*\.\s*)?(?:getElementById|querySelector)\(\s*(['"`])#?([^'"`$]+)\2\s*\)/gu)) {
                const objectProperty = entry[1];
                const id = entry[3];
                if (!objectProperty || !id || !selectHasAlternativeOption(html, id))
                    continue;
                const escapedObject = objectName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                const escapedObjectProperty = objectProperty.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                const handlerPattern = new RegExp(String.raw `\b${escapedObject}\s*\.\s*${escapedObjectProperty}\s*\.\s*on(?:change|input)\s*=\s*(?:\(\s*)?([A-Za-z_$][\w$]*)(?:\s*\))?\s*=>\s*\{`, "gu");
                for (const handler of source.content.matchAll(handlerPattern)) {
                    ...
                    if (!eventVariable || handlerBrace < 0 || handlerEnd < 0)
                        continue;
                    const body = source.content.slice(handlerBrace + 1, handlerEnd);
                    const escapedEvent = eventVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                    const assignment = new RegExp(String.raw `\bstate\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*${escapedEvent}\s*\.\s*target\s*\.\s*value\b`, "u").exec(body);
                    const property = assignment?.[1];
                    if (property) {
                        assignments.push({
                            id,
        ...
        const directBindingPattern = /\b(?:document\s*\.\s*)?getElementById\(\s*(['"`])([^'"`$]+)\1\s*\)\s*\.\s*addEventListener\(\s*(['"`])change\3\s*,\s*(?:\(\s*)?([A-Za-z_$][\w$]*)(?:\s*\))?\s*=>\s*\{([\s\S]{0,1536}?)\}\s*\)/gu;
        for (const binding of source.content.matchAll(directBindingPattern)) {
            const id = binding[2];
            const eventVariable = binding[4];
            const body = binding[5] ?? "";
            if (!id || !eventVariable || !selectIdExists(html, id))
                continue;
            const escapedEvent = eventVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            const assignment = new RegExp(String.raw `\bstate\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*${escapedEvent}\s*\.\s*target\s*\.\s*value\b`, "u").exec(body);
            const property = assignment?.[1];
            if (property)
                assignments.push({ id, property, index: binding.index ?? 0 });
        }
        const sharedBindingPattern = /\b([A-Za-z_$][\w$]*)\s*\.\s*addEventListener\(\s*(['"`])change\2\s*,[\s\S]{0,256}?\{([\s\S]{0,4096}?)\}\s*\)/gu;
        for (const binding of source.content.matchAll(sharedBindingPattern)) {
            ...
            const idVariable = new RegExp(String.raw `\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*${escapedSelect}\s*\.\s*id\b`, "u").exec(body)?.[1];
            if (!idVariable)
                continue;
            const escapedIdVariable = idVariable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            const branchPattern = new RegExp(String.raw `(?:if|else\s+if)\s*\(\s*${escapedIdVariable}\s*={2,3}\s*(['"\x60])([^'"\x60$]+)\1\s*\)\s*state\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*${escapedSelect}\s*\.\s*value\b`, "gu");
            for (const branch of body.matchAll(branchPattern)) {
                const id = branch[2];
                const property = branch[3];
                if (!id || !property || !selectIdExists(html, id))
                    continue;
                assignments.push({
                    id,
                    property,
        ...
        for (const select of html.matchAll(/<select\b([^>]*)>[\s\S]*?<\/select>/giu)) {
            ...
            if (!id)
                continue;
            const escapedId = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            const direct = new RegExp(String.raw `(?:document\s*\.\s*)?getElementById\(\s*(['"\x60])${escapedId}\1\s*\)\s*\.\s*addEventListener\(\s*(['"\x60])change\2[\s\S]{0,768}?\bstate\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*[A-Za-z_$][\w$]*\s*\.\s*target\s*\.\s*value\b`, "u").exec(source.content);
            const branch = new RegExp(String.raw `(?:if|else\s+if)\s*\(\s*[A-Za-z_$][\w$]*\s*={2,3}\s*(['"\x60])${escapedId}\1\s*\)\s*state\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*[A-Za-z_$][\w$]*\s*\.\s*value\b`, "u").exec(source.content);
        ...
        for (const assignment of assignments) {
            if (renderedDataReadsFlatStateProperty(combined, assignment.property))
                continue;
            errors.push(`static.filter_value_unused: Select #${assignment.id} writes state.${assignment.property}, but rendered-data code never reads that filter property. Evidence: ${evidenceAt(source, assignment.index)}`);
        }
    }
    return unique(errors);
}
function renderedDataReadsFlatStateProperty(source, property) {
    const escapedProperty = property.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
function selectHasAlternativeOption(html, id) {
    for (const select of html.matchAll(SELECT_TAG_PATTERN)) {
        if (attributeValue(select[1] ?? "", "id") !== id)
            continue;
        return enabledStaticSelectOptionCount(select[0]) >= 2;
    }
    return false;
}
function withoutAdvisoryCalls(source) {
    const ranges = [];
...
function declaredFunctionBodies(source) {
    const functions = [];
    const declarations = [
        ...source.matchAll(/\bfunction\s+([A-Za-z_$][\w$]*)\s*\([^)]*\)\s*\{/gu),
        ...source.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:async\s*)?(?:\([^)]*\)|[A-Za-z_$][\w$]*)\s*=>\s*\{/gu),
...
function matchingJavaScriptBrace(source, openBrace) {
    return matchingJavaScriptDelimiter(source, openBrace, "{", "}");
}
function matchingJavaScriptDelimiter(source, openIndex, open, close) {
    let depth = 0;
...
function scriptReferencesId(source, id) {
    const escaped = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const directReference = new RegExp(String.raw `(?:getElementById\(\s*['"\x60]${escaped}['"\x60]\s*\)|(?:querySelector|querySelectorAll)\(\s*['"\x60][^'"\x60]*#${escaped}\b)`, "u").test(source);
    if (directReference)
        return true;
...
function collectReferencedIds(scripts, html) {
    const ids = new Map();
    const inlineScripts = [...html.matchAll(SCRIPT_BLOCK_PATTERN)]
        .filter((match) => !staticHtmlAttributeValue(match[1] ?? "", "src"))
...
function extractReferencedIds(source) {
    ...
    for (const match of source.matchAll(QUERY_SELECTOR_PATTERN)) {
        ...
        for (const id of idsFromSelector(selector))
            ids.add(id);
    }
...
function visibleLoadingIds(html) {
    const ids = [];
    for (const element of htmlElements(html)) {
        if (!element.id || !/\bloading\b/i.test(element.className))
            continue;
        if (isHiddenByDefault(element.className, element.style))
            continue;
        ids.push(element.id);
    }
    return unique(ids);
}
function metricPlaceholderIds(html) {
    const ids = [];
    for (const element of htmlElements(html)) {
        if (!element.id || !element.text.includes("--"))
            continue;
        const signal = `${element.id} ${element.className}`;
        if (!/(kpi|metric|value|yield|count|output|loss|updated)/i.test(signal))
            continue;
        ids.push(element.id);
    }
    return unique(ids);
...
function attributeValue(attrs, name) {
    const pattern = name === "id" ? ID_ATTRIBUTE_PATTERN : name === "class" ? CLASS_ATTRIBUTE_PATTERN : STYLE_ATTRIBUTE_PATTERN;
    pattern.lastIndex = 0;
    const match = pattern.exec(attrs);
    return match?.[2]?.trim() ?? "";
}
function isHiddenByDefault(className, style) {
    return (/\b(d-none|hidden|visually-hidden|sr-only)\b/i.test(className) ||
...
//# sourceMappingURL=static-preview-quality-gate.js.map