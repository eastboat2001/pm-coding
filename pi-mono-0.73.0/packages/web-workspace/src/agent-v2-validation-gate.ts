import {
	type AgentV2BlueprintProjectSource,
	inspectAgentV2BlueprintQuality,
} from "./agent-v2-blueprint-quality-gate.js";
...
import type { AgentV2ProductBlueprint } from "./agent-v2-types.js";
...
export interface RunAgentV2StaticValidationGateInput {
	...
	productBlueprint?: AgentV2ProductBlueprint;
	projectSources?: readonly AgentV2BlueprintProjectSource[];
	signal?: AbortSignal;
}
...
export async function runAgentV2StaticValidationGate(
	...
	const failures: AgentV2ValidationFailure[] =
		taskResult.task === "build_static" && taskResult.status === "failed"
			? [classifyBuildRunnerFailure(taskResult, input.taskId)]
			: rawErrors.map((message) =>
					classifyStaticValidationFailure(
						message,
						input.taskId,
						input.projectSources ?? [],
						input.productBlueprint,
					),
				);
...
function classifyExplicitDeterminismFailure(input: {
	...
	projectSources: readonly AgentV2BlueprintProjectSource[];
	taskId: string;
}): AgentV2ValidationFailure | undefined {
	const requirement = input.blueprint?.items.find(
		(item) =>
			!DEFERRED_DETERMINISM_REQUIREMENT_PATTERN.test(item.text) &&
	...
	if (!randomEvidence) return undefined;
	...
	return createFailure({
		...
		message:
			"Static validation found unseeded Math.random() despite an explicit deterministic or reproducible product requirement.",
		retryable: true,
		source: "static_quality",
		taskId: input.taskId,
...
function executableRandomEvidence(
	projectSources: readonly AgentV2BlueprintProjectSource[],
): { path: string; line: number } | undefined {
	for (const source of projectSources) {
		if (!EXECUTABLE_PROJECT_SOURCE_PATTERN.test(source.path)) continue;
...
function rawWarningsFor(result: ProjectTaskResult): string[] {
...
function classifyStaticValidationFailure(
	message: string,
	taskId: string,
	projectSources: readonly AgentV2BlueprintProjectSource[],
	productBlueprint?: AgentV2ProductBlueprint,
	...
	if (normalized.startsWith("Static preview quality gate: ")) {
		return classifyQualityFailure(
			message,
			normalized.slice("Static preview quality gate: ".length),
			taskId,
			projectSources,
			productBlueprint,
		);
	...
	if (normalized.startsWith("Static preview smoke gate: ")) {
		return classifySmokeFailure(
			message,
			normalized.slice("Static preview smoke gate: ".length),
			taskId,
			projectSources,
			productBlueprint,
		);
...
function classifyQualityFailure(
	...
	const unwiredSelect = message.match(
		/^Select control (#\S+) is never referenced by local JavaScript and cannot affect rendered data\.$/,
	);
	if (unwiredSelect) {
		...
		return createFailure({
			...
			data: {
				...
				...(highConfidence
					? {
							sourceEvidence:
								"The PM Product Blueprint explicitly scopes this exact filter to all charts/visuals, but project source never references the control.",
						}
					: {}),
			},
	...
	const unusedFilterValue = message.match(
		/^static\.filter_value_unused: Select (#\S+) reads its value into ([A-Za-z_$][\w$]*) but never uses that value\. Evidence: (.+):(\d+) (.+)$/u,
	);
	...
	const inertDelegatedFilter = message.match(
		/^static\.filter_value_unused: Select (#\S+) updates (this\.filters\.[A-Za-z_$][\w$]*), but getFilteredData\(\) returns this\.dataset unchanged and the redraw subscriber does not consume filter arguments\. Evidence: (.+):(\d+) (.+)$/u,
	);
	...
	const unusedFilterState = message.match(
		/^static\.filter_value_unused: Select (#\S+) writes ([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)+)(?: [^,]+)?, but rendered-data code never reads that filter property\. Evidence: (.+):(\d+) (.+)$/u,
	);
	...
	const filterStateKeyMismatch = message.match(
		/^static\.filter_state_key_mismatch: .+ Controls: (.+?)\. Evidence: (.+)$/u,
	);
	if (filterStateKeyMismatch) {
		...
		return createFailure({
			...
			message:
				"Static validation found a shared select handler that writes hyphenated control values to nonexistent render-state keys.",
			retryable: true,
			source: "static_quality",
			taskId,
	...
	const resetOnlyFilter = message.match(
		/^static\.filter_value_unused: Select (#\S+) is only reset and is never read by an applicable filter handler\. Evidence: (.+):(\d+) (.+)$/u,
	);
	if (resetOnlyFilter) {
	...
	const genericHandlerUnusedFilter = message.match(
		/^static\.filter_value_unused: Select (#\S+) is bound through a generic change handler, but its value is never read by rendered-data code\. Evidence: (.+):(\d+) (.+)$/u,
	);
	...
	const omittedBindingMapFilter = message.match(
		/^static\.filter_value_unused: Select (#\S+) is omitted from the explicit filter binding map and cannot update declared (state\.filters\.[A-Za-z_$][\w$]*)\. Evidence: (.+):(\d+) (.+)$/u,
	);
	...
	const horizontalOverflow = message.match(
		/^static\.page_horizontal_overflow: .+ (?:Grid|Target): (.+?)\. (?:Canvases: (.+?)\. )?Evidence: (.+)$/u,
	);
	if (horizontalOverflow) {
		...
		return createFailure({
			...
			message:
				"Static validation found source-proven intrinsic content that can force the page beyond the viewport.",
			retryable: true,
			source: "static_quality",
			taskId,
...
function classifyCanvasQualityFailure(
	sourceMessage: string,
	message: string,
	taskId: string,
): AgentV2ValidationFailure | undefined {
	const code = message.match(
		/^(static\.(?:canvas_css_bitmap_mismatch|canvas_layout_unbounded|canvas_resize_unhandled|svg_coordinate_space_mismatch)):/u,
	)?.[1];
	...
	const descriptions: Record<string, string> = {
		"static.canvas_css_bitmap_mismatch":
			"Static validation found chart canvas bitmap dimensions scaled for DPR without explicit CSS display dimensions.",
		"static.canvas_layout_unbounded":
			"Static validation found a chart canvas without a dedicated bounded drawing viewport.",
		"static.canvas_resize_unhandled":
			"Static validation found a responsive chart canvas with no ResizeObserver or resize redraw handler.",
		"static.svg_coordinate_space_mismatch":
			"Static validation found a responsive chart SVG whose drawing coordinate height conflicts with its bounded CSS viewport.",
	};
	return createFailure({
		code,
...
function classifySmokeFailure(
	...
	const invalidRenderedData = message.match(
		/^Runtime smoke gate: dashboard data surface (\S+) rendered invalid token (undefined|NaN) (.+?)\. Evidence: (.+)$/u,
	);
	...
	const defaultFilterInconsistent = message.match(
		/^Runtime smoke gate: applying unchanged default filters replaced representative KPI data with an empty result(?:; (.+):(\d+) filter predicate reads missing fixture field ([A-Za-z_$][\w$]*))?\.$/,
	);
	if (defaultFilterInconsistent) {
		...
		return createFailure({
			...
			message:
				"Static runtime validation found that applying the unchanged default filters replaces representative dashboard data with an empty result.",
			retryable: true,
			source: "static_smoke",
			taskId,
			path: detectedPath,
	...
	const emptyStateChartMismatch = message.match(
		/^Runtime smoke gate: dashboard rendered explicit empty state in (\S+) while chart surfaces (.+?) still contained data (after .+)\.$/u,
	);
	...
	const controlNoEffect = message.match(
		/^Runtime smoke gate: (deterministic fixture )?select (#\S+) changed value but did not change rendered metrics, chart data, results, or empty state\.$/,
	);
	const partialDashboardUpdate = message.match(
		/^Runtime smoke gate: deterministic global select (#\S+) changed some dashboard data but left synchronized surfaces unchanged: (.+)\.$/u,
	);
...
function blueprintRequiresAllChartsFilterScope(
	blueprint: AgentV2ProductBlueprint | undefined,
	selector: string,
	projectSources: readonly AgentV2BlueprintProjectSource[],
): boolean {
	if (!blueprint || blueprint.sourceDocuments.length === 0) return false;
...
function normalizeControlIdentity(selector: string, projectSources: readonly AgentV2BlueprintProjectSource[]): string {
	const id = selector.startsWith("#") ? selector.slice(1) : "";
	const fragments = [selector];
	if (id) {
		const escapedId = id.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
		for (const source of projectSources) {
			const tag = new RegExp(String.raw`<select\b(?=[^>]*\bid\s*=\s*(["'])${escapedId}\1)([^>]*)>`, "iu").exec(
				source.content,
			);
			if (tag) {
			...
			fragments.push(
				new RegExp(
					String.raw`<label\b[^>]*\bfor\s*=\s*(["'])${escapedId}\1[^>]*>([\s\S]{0,300}?)<\/label>`,
					"iu",
				).exec(source.content)?.[2] ?? "",
			);
		}
	}
...
function controlBindingSource(
	...
	const directBinding = new RegExp(
		String.raw`(?:getElementById\(\s*['"\x60]${escapedId}['"\x60]\s*\)|querySelector(?:All)?\(\s*['"\x60]#${escapedId}['"\x60]\s*\))`,
		"u",
	);
...
function delegatedSelectBinding(source: string, id: string): RegExpExecArray | null {
	const dynamicCollectionBinding = dynamicSelectCollectionBinding(source, id);
	if (dynamicCollectionBinding) return dynamicCollectionBinding;
	...
	const selectsCollection =
		/querySelectorAll\s*\(\s*(["'`])[^"'`]*\bselect\b[^"'`]*\1\s*\)/iu.test(source) ||
		/getElementsByTagName\s*\(\s*(["'`])select\1\s*\)/iu.test(source);
	const changeListener =
		/addEventListener\s*\(\s*(["'`])(?:change|input)\1/iu.test(source) || /on(?:change|input)\s*=/iu.test(source);
	...
	if (!hasDelegatedSelectListener) return null;
	const escapedId = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
function dynamicSelectCollectionBinding(source: string, id: string): RegExpExecArray | null {
	const escapedId = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
	for (const declaration of source.matchAll(
		/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*\[([\s\S]{1,2048}?)\]\s*;/gu,
	)) {
		const collection = declaration[1];
		const items = declaration[2] ?? "";
		if (!collection || !new RegExp(`(['"\\x60])${escapedId}\\1`, "u").test(items)) continue;
		const escapedCollection = collection.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
		for (const loop of source.matchAll(
			new RegExp(
				String.raw`\b${escapedCollection}\s*\.\s*forEach\s*\(\s*([A-Za-z_$][\w$]*)\s*=>\s*\{([\s\S]{1,8192}?)\}\s*\)`,
				"gu",
			),
		)) {
...
function sourceEvidencePath(value: string | undefined): string | undefined {
	if (!value) return undefined;
	const path = /^(.*?):\d+(?:\s|$)/u.exec(value)?.[1]?.trim();
	if (!path || path.startsWith("inline script ")) return undefined;
