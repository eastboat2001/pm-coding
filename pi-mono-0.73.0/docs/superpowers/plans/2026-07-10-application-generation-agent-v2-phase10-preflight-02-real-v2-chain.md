...
export interface AgentV2OutboxDeliveryInput {
	intentId: string; ownerId: string; leaseAttempt: number; deliveredAt: string;
}
export interface AgentV2OutboxRescheduleInput {
	intentId: string; ownerId: string; leaseAttempt: number; availableAt: string; errorCode: string;
...
Every successful execution mutation consumes the expected revision. Run/task revisions are canonical UTC millisecond timestamps whose string must equal `new Date(value).toISOString()`. `input.updatedAt` must be strictly later than the expected/current run revision, becomes the new run `updatedAt` even for artifact/validation/diagnostic/event-only writes, and every changed task must receive a canonical revision strictly later than its expectation. Equal, malformed, older or previously used historical run/task revisions are rejected before any write, so `T0 -> T1 -> T0` cannot restore a stale expectation. Transition/diagnostic and every other composite child must match the target `(clientId,runId)` before the first write; cross-run children always fail atomically.

The same revision invariant applies at every durable run boundary, not only execution. Start accepts only canonical run and initial-task `createdAt`/`updatedAt` revisions with each `updatedAt >= createdAt`, so it cannot establish an adapter-dependent baseline. Transition requires `update.updatedAt` to be canonical and strictly later than the exact expected/current run revision, including phase-only transitions. Cancel requires canonical `cancelledAt` strictly later than the exact expected/current run revision and persists it as the terminal run/event revision. SQLite and PostgreSQL must reject malformed, noncanonical, equal, older and historical-reuse revisions identically before any run/task/event/outbox/diagnostic write.
...
Start replay compares only the complete immutable canonical start contract, not the run's later mutable status/phase/worker/revision: run identity/input/model/created identity, bootstrap version/checksum, every blob/reference/document/task/artifact/diagnostic, queue name, seq 1/2 event type/payload/time, and the exact start-owned intent set must match semantically. A canonical SHA-256 `startReplayFingerprint` may summarize that evidence, but it is persisted only in the server-side `agent_v2_bootstraps.bootstrap_checksum` column (the caller-provided bootstrap checksum is one fingerprint input); it must never appear in durable event payloads, event APIs, live intents, SSE/browser projections or logs. It returns only start-owned intent IDs; later run intents are never included. Any field difference conflicts with zero writes. PostgreSQL serializes concurrent missing-row starts by `(clientId,runId)` with a transaction-scoped advisory lock or equivalent, then re-reads so two identical concurrent starts produce one create plus one replay rather than a PK error.

Cancel replay likewise validates the complete canonical cancel request/result—target identity, expected snapshot, queue/token, cancellation time/reason, terminal run state, cancel event and both cancel/live intents. Only an identical request returns `replayed:true`; any difference or missing intent conflicts with zero writes.

Lease ordering is `(available_at,created_at,intent_id)`. A lease atomically selects pending or expired-leased rows and CASes owner/status/expiry, incrementing `attemptCount` on every successful lease. That returned `attemptCount` is the lease generation and callers must echo it as `leaseAttempt` for delivery/reschedule; the SQL CAS includes owner, leased status, exact attempt generation and an unexpired lease at the supplied operation time. A delayed callback from an expired or re-leased generation returns `lease_lost`, including when the stable owner ID is reused. Reschedule returns `dead_letter` when `attemptCount >= maxAttempts`, otherwise pending. Validate owner, positive generation/limit/TTL/maxAttempts and timestamps.
...
- [ ] **Step 1: Write SQLite RED tests** for full rollback at blob/reference/bootstrap/planning/outbox and every cancel/transition/execution/diagnostic composite fault point; immutable input reads; complete start/cancel semantic replay after later run progress; private start fingerprint with durable event/SSE projections proving no raw evidence/checksum/fingerprint; canonical dedupe/reference conflict; exact start-owned intents; cross-run child rejection; contiguous event allocation and intent mapping; lease ordering/recovery/generation/expiry/dead-letter; canonical start baselines plus strictly-monotonic transition/cancel/execution run/task revisions including malformed/noncanonical/equal/older/history-ABA rejection; and per-table zero dependent writes.
- [ ] **Step 2: Write factory/contract/reset/boundary and transaction-shape RED tests.** Prove concrete production stores and factory outputs satisfy `AgentV2DurableCommitStore & AgentV2OutboxStore`, `runtime-infra`/root exports are available without widening `RuntimeStore`, reset covers outbox/input/bootstrap tables, and each composite success/fault path records exactly one transaction boundary with no nested public transaction.
- [ ] **Step 3: Write required real PostgreSQL durable integration RED tests** reusing Task 1's temporary-schema helper; missing `PI_TEST_POSTGRES_URL` fails, never skips. Use two real fixed-search-path connections and barriers: while owner A holds an uncommitted lease, owner B must obtain a different intent before the timeout via `FOR UPDATE SKIP LOCKED`; leases never overlap. Separately race two identical missing-row starts and prove one create plus one replay with no PK error/duplicates. Cover every run CAS field, missing/duplicate/cross-run/ABA task expectations, invalid start baseline revisions, phase-only transition and cancel equal/malformed/noncanonical/older/history-ABA revisions, execution run/task revisions, lease generation/expiry ABA, cross-run diagnostic rollback, private fingerprint visibility, and composite fault checks that assert every dependent run/event/outbox/diagnostic/artifact/validation/task table has zero residue. Close stores before the admin-only temporary-schema drop and never touch `public` or `.env`.
