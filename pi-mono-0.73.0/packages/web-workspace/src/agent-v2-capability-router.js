...
const FRONTEND_BUILD_PATTERNS = [/\breact\b/i, /\bvue\b/i, /\bfrontend\b/i, /\bcomponent library\b/i];
const FULL_STACK_PATTERNS = [/\bfull[ -]?stack\b/i];
const DATABASE_PATTERNS = [/\bpostgres(?:ql)?\b/i, /\bdatabase\b/i, /\bsql\b/i, /\bprisma\b/i, /\borm\b/i];
const AUTH_PATTERNS = [/\bauth\b/i, /\blogin\b/i, /\bsign[ -]?in\b/i, /\buser roles?\b/i, /\bpermissions?\b/i];
const BACKEND_PATTERNS = [/\bapi routes?\b/i, /\bbackend\b/i, /\bserver\b/i, /\bendpoint\b/i];
const API_PATTERNS = [
...
export const STATIC_APP_V2_PLATFORM_CONTRACT = Object.freeze({
    ...
    userVisibleContract: "This workspace ships a static app contract: frontend-only delivery, mock data allowed, and server-dependent features must be framed as a static simulation.",
    metadata: {
...
export function routeAgentV2Capabilities(input) {
    ...
    collectUnsupportedEvidence(objective, FULL_STACK_PATTERNS, "backend_server", "Explicit full-stack scope requires server-side runtime behavior outside the static platform contract.", evidence, unsupportedCapabilities);
    collectUnsupportedEvidence(objective, DATABASE_PATTERNS, "database_runtime", "Requires runtime persistence that the static platform contract does not provide.", evidence, unsupportedCapabilities);
    collectUnsupportedEvidence(objective, AUTH_PATTERNS, "server_auth", "Requires authentication or role enforcement that needs a server-side authority.", evidence, unsupportedCapabilities);
    collectUnsupportedEvidence(objective, API_PATTERNS, "backend_server", "API delivery requires backend request handling beyond a static frontend bundle.", evidence, unsupportedCapabilities);
    collectUnsupportedEvidence(objective, BACKEND_PATTERNS, "backend_server", "Requires backend request handling beyond a static frontend bundle.", evidence, unsupportedCapabilities);
    collectUnsupportedEvidence(objective, FILE_UPLOAD_PATTERNS, "file_upload_runtime", "Requires a file upload runtime and persistence flow that the static platform contract does not provide.", evidence, unsupportedCapabilities);
    collectUnsupportedEvidence(objective, SCHEDULED_JOB_PATTERNS, "scheduled_jobs", "Requires scheduled or background job execution beyond a static frontend bundle.", evidence, unsupportedCapabilities);
    collectUnsupportedEvidence(objective, EXTERNAL_INTEGRATION_PATTERNS, "external_integration_runtime", "Requires webhook handling or third-party integration runtime behavior outside the static platform contract.", evidence, unsupportedCapabilities);
    ...
    if (unsupportedCapabilities.size > 0) {
        ...
        return buildDecision({
            ...
            rationale: "The request includes backend-only capabilities that cannot run under the static app platform contract, so the delivery mode must stay explicit about simulation boundaries.",
    ...
    const deliveryMode = evidence.some((entry) => entry.capability === "build_static_frontend")
        ? "build_static_frontend"
        : "static_app";
    return buildDecision({
        deliveryMode,
...
function buildDecision(input) {
    const contractText = input.deliveryMode === "static_simulation"
...
function matchesAny(objective, patterns) {
    return patterns.some((pattern) => pattern.test(objective));
}
function collectEvidence(objective, patterns, capability, reason, evidence) {
...
function collectUnsupportedEvidence(objective, patterns, capability, reason, evidence, unsupportedCapabilities) {
    let matched = false;
    for (const pattern of patterns) {
        const match = objective.match(pattern);
        if (match) {
...
//# sourceMappingURL=agent-v2-capability-router.js.map