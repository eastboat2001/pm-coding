...
export class WorkspaceDiagnosticExportService {
    ...
    async exportArchive(request) {
        ...
        for (const run of context.runs) {
            ...
            add(jsonEntry(`runtime/run-events/${runId}.summary.json`, "agent-v2-run-events-summary", async () => summarizeRunEvents(run.runId, await this.runtimeDb.listAgentV2RunEvents(context.clientId, run.runId, 0))));
            add(ndjsonEntry(`runtime/run-events/${runId}.events.ndjson`, "agent-v2-run-events", () => this.runtimeDb.listAgentV2RunEvents(context.clientId, run.runId, 0)));
            add(jsonEntry(`agent-v2/diagnostics/${runId}.summary.json`, "agent-v2-diagnostics-summary", async () => summarizeAgentV2Diagnostics(run.runId, await this.runtimeDb.listAgentV2Diagnostics(context.clientId, run.runId))));
            add(ndjsonEntry(`agent-v2/diagnostics/${runId}.diagnostics.ndjson`, "agent-v2-diagnostics", () => this.runtimeDb.listAgentV2Diagnostics(context.clientId, run.runId)));
        }
    ...
    async resolveContext(request) {
        ...
        if (request.runId && !requestedRun)
            throw new Error("Agent v2 run not found.");
...
async function collectAgentV2Diagnostics(runtimeDb, clientId, runs) {
    const diagnosticsByRunId = {};
    for (const run of runs) {
...
async function buildDiagnosticOverview(input) {
    ...
    const agentV2DiagnosticsByRunId = await collectAgentV2Diagnostics(input.runtimeDb, input.context.clientId, input.context.runs);
...
async function buildRunOverviews(runtimeDb, context, diagnosticEvents, agentV2DiagnosticsByRunId) {
    const overviews = [];
    for (const run of context.runs) {
...
async function arrayFromMaybeAsync(values) {
    const result = [];
...
function countAgentV2DiagnosticCodes(events) {
    const counts = {};
    for (const event of events) {
...
function agentV2RunSessionId(run) {
...
async function summarizeRunEvents(runId, events) {
    let totalEvents = 0;
    let payloadBytes = 0;
...
async function summarizeAgentV2Diagnostics(runId, diagnostics) {
