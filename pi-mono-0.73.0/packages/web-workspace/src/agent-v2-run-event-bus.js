...
export class InMemoryAgentV2RunEventBus {
    ...
    assertOpen() {
        if (this.closed)
            throw new Error("Agent v2 run event bus is closed");
    }
}
export class RedisAgentV2RunEventBus {
    activeReads = 0;
    ...
    async read(request) {
        ...
        try {
            ...
            const result = await blockingClient.xRead({ key: agentV2RunEventStreamKey(request), id: `${request.afterSeq}-0` }, { BLOCK: request.blockMs ?? DEFAULT_READ_BLOCK_MS, COUNT: DEFAULT_READ_COUNT });
            if (this.closed || request.signal?.aborted) {
                return [];
            }
...
function isAgentV2LiveRunEvent(value) {
