...
Every successful execution mutation consumes the expected revision: `input.updatedAt` must be strictly different from the expected run revision, becomes the new run `updatedAt` even for artifact/validation/diagnostic/event-only writes, and every changed task must receive a new `updatedAt` rather than reuse its expectation. A non-advancing run/task revision is rejected with zero writes. Transition/diagnostic and every other composite child must match the target `(clientId,runId)` before the first write; cross-run children always fail atomically.
...
- [ ] **Step 1: Write SQLite RED tests** for full rollback at blob/reference/bootstrap/planning/outbox and every cancel/transition/execution/diagnostic composite fault point; immutable input reads; complete start/cancel semantic replay after later run progress; canonical dedupe/reference conflict; exact start-owned intents; cross-run child rejection; contiguous event allocation and intent mapping; lease ordering/recovery/generation/expiry/dead-letter; full run/task ABA revision consumption; and zero dependent writes.
